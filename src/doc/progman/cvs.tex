...
\item Fortran source files should be named with a \file{.F} suffix so that
they will be processed by \file{psipp}.
...
\item \shellvar{\$PSI3}\file{/src/util} -- Source code for the utility
  programs \module{psipp} and \module{tocprint}.  (Note that the
  \module{tmpl} module is no longer used and will eventually
  disappear.)
