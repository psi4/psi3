...
single file.  Interestingly, there is a UNIX program called
\file{fsplit} which splits gigantic Fortran files into smaller files,
one file for each subroutine.\footnote{Often the aforementioned giant
Fortran files also have line numbering information out past the 72nd
column.  This can cause problems when compiling the programs as a part
of \PSIthree.  Characteristically, Ed Seidl has written a program,
called \module{fortran\_sux}, which deletes any characters after the
72nd column and any trailing spaces.}
...
lines to 80 characters or less!\footnote{Of course in Fortran, at
least older Fortran, you need to keep lines less than 72 characters
long.}  This means that you should use about 2-4 spaces per
indentation level.  Anything more than this will cause problems!
