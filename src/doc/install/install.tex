...
\section{Architecture-specific issues}
\begin{itemize}
\item Reentrant versions of C, C++, and Fortran
compilers should be used whenever possible.
\item AIX 4.3/5.x in 64-bit environment:
AIX {\tt lex} should be used instead of {\tt GNU flex}.
This can be achieved with the following configure options:
{\tt --with-lex=/bin/lex --with-lexlib=/usr/lib/libl.a}
\item AIX 4.3/5.x in 64-bit environment:
