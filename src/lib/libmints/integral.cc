            ...
            for (uskk=0; uskk<=usjj; uskk++) {
                for (usll=0; usll<=uskk; usll++) {
                    ...
                    for (int upk=0; upk < num_unique_pk; ++upk) {
                        ...
                        int ni = bs1->shell(usi)->nfunction(0);
                        int nj = bs2->shell(usj)->nfunction(0);
                        int nk = bs3->shell(usk)->nfunction(0);
                        int nl = bs4->shell(usl)->nfunction(0);
                                ...
                                for (int jj=0; jj <= jjmax; ++jj) {
                                    ...
                                    for (int kk=0; kk <= kkmax; ++kk) {
                                        ...
                                        for (int ll=0; ll <= llmax; ++ll) {
                                            // Save this integral
                                            Integral integral;
                                            integral.i = ii + bs1->shell(usi)->function_index();
                                            integral.j = jj + bs2->shell(usj)->function_index();
                                            integral.k = kk + bs3->shell(usk)->function_index();
                                            integral.l = ll + bs4->shell(usl)->function_index();
                                            integral.index = ll+nl*(kk+nk*(jj+nj*ii));
                                            q.unique_integrals_.push_back(integral);
                                        }
                                    }
                                }
                            }
                        ...
                        // If we want to write a cscf pk-compatible integral system we would want
                        // to negate i on the last integral of q if we are on the last upk iteration 
                        // of num_unique_pk.
                        //
                        // if (upk == num_unique_pk - 1) {
                        //     int last = q.unique_integrals_.size() - 1;
                        //     q.unique_integrals_[last] = -q.unique_integrals_[last];
                        // }
