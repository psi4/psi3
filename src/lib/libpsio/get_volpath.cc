...
void
PSIO::get_volpath(unsigned int unit, unsigned int volume, char **path)
{
  std::string kval;
  char volumeX[20];
  ...
  if(!kval.empty()) { *path = strdup(kval.c_str()); return; }
...
extern "C" {
  ...
  */
  int psio_get_volpath_default(unsigned int volume, char **path)
  {
    ...
    if(!kval.empty()) { *path = strdup(kval.c_str()); return(1); }
