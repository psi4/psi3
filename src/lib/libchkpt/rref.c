...
**   takes no arguments.
**
**   returns: double **rref  A 3x3 matrix describing the rotation back to the reference frame,
** 			      Reference frame is a coordinate system defined by the "raw"
**			      geometry specification (either Z-matrix or geometry array
**			      in input.dat or chkpt). Can be used to transform quantities
**			      corresponding to different but similar calculations
**			      (gradients at displaced geometries) to a common frame.
*/
...
double **chkpt_rd_rref(void)
{
  ...
  psio_read_entry(PSIF_CHKPT, "::Transmat to reference frame", (char *) Rref[0], 
		  sizeof(double)*9);
...
**
** arguments: 
**   \params double **rref  A 3x3 matrix describing the rotation back to the reference frame,
** 			      Reference frame is a coordinate system defined by the "raw"
**			      geometry specification (either Z-matrix or geometry array
**			      in input.dat or chkpt). Can be used to transform quantities
**			      corresponding to different but similar calculations
