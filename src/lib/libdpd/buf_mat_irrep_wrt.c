...
/* dpd_buf_mat_irrep_wrt(): Writes an entire irrep from disk into a dpd
** two-electron buffer using the "rules" specified when the buffer was
** initialized by dpd_buf_init().
**
** Arguments:
**   struct dpdbuf *Buf: A pointer to the dpdbuf where the data is
**                       stored.
**   int irrep: The irrep number to be writted.
**   int print_flag: Boolean which indicates whether the irrep will be
**                   printed.
**   FILE *outfile: The formatted output file stream.
**
** The case numbers are the same as those from buf_mat_irrep_rd().
			  ...
			  FILE *outfile)
{
  ...
  if(Buf->anti) {
      ...
      fprintf(outfile,   "\tback to original DPD file!\n");
      exit(1);
    }
  ...
  case 12: /* No change in pq or rs */
      ...
      dpd_file_mat_irrep_wrt(&(Buf->file), irrep, print_flag, outfile);
  ...
  case 21: /* Pack pq; no change in rs */
	  ...
	  for(rs=0; rs < coltot; rs++) {
	      ...
	      /* Assign the value */
	      Buf->file.matrix[irrep][0][rs] = value;
	    }

	  /* Write out the row */
	  dpd_file_mat_irrep_row_wrt(&(Buf->file), irrep, pq);
	}
  ...
  case 23: /* Unpack pq; no change in rs */
	  ...
	  for(rs=0; rs < coltot; rs++) {
	      ...
	      /* Assign the value */
	      Buf->file.matrix[irrep][0][rs] = permute*value;
	    }
