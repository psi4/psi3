				...
				int num_pq, int print_flag, FILE *outfile)
{
  ...
  case 12: /* No change in pq or rs */
      ...
      dpd_file_mat_irrep_wrt_block(&(Buf->file), irrep, start_pq, num_pq,
				   print_flag, outfile);
  ...
  case 21: /* Pack pq; no change in rs */
	  ...
	  /* Write out the row */
	  dpd_file_mat_irrep_row_wrt(&(Buf->file), irrep, filepq);
	}

      /* No printing for this function yet */
      if(0) dpd_buf_mat_irrep_print(Buf, irrep, outfile);
