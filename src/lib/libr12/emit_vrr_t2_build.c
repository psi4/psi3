...
int emit_vrr_t2_build(int old_am, int new_am, int max_class_size)
{
  FILE *code;
  int i, j, k, l, f;
  ...
  for(la=0;la<=new_am;la++) {
    ...
    for(lc=lc_min;lc<=lc_max;lc++) {
      ...
      fprintf(vrr_header,"#define _T2_BUILD_%c0%c0(vp,i0,i1,i2,i3,i4) {",am_letter[la],am_letter[lc]);
	...
	fprintf(vrr_header," tmp = _t2_build_%c0%c0_0(vp,i0,i1,i2,i3,i4); \\\n",am_letter[la],am_letter[lc]);
	for(f=1;f<num_subfunctions;f++)
	  fprintf(vrr_header," tmp = _t2_build_%c0%c0_%d(tmp,i0,i1,i2,i3,i4); \\\n",am_letter[la],am_letter[lc],f);
	fprintf(vrr_header,"}\n");
	for(f=0;f<num_subfunctions;f++)
	  fprintf(vrr_header, "double *_t2_build_%c0%c0_%d(double *, const double *, const double *, const double *, const double *, const double *);\n",
		  am_letter[la],am_letter[lc],f);
      }
      else {
	split = 0;
	fprintf(vrr_header," _t2_build_%c0%c0(vp,i0,i1,i2,i3,i4);}\n",am_letter[la],am_letter[lc]);
	fprintf(vrr_header,"void _t2_build_%c0%c0(double *, const double *, const double *, const double *, const double *, const double *);\n",
		am_letter[la],am_letter[lc]);
      }

      sprintf(function_name,"t2_build_%c0%c0",am_letter[la],am_letter[lc]);
      sprintf(code_name,"t2_build_%c0%c0.c",am_letter[la],am_letter[lc]);
...
void declare_localv(int lc, FILE *code)
{
  ...
  fprintf(code,"  extern double AC[3], CD[3], CDdotCA;\n");
...
void define_localv(int lc, FILE *code)
{
  ...
  fprintf(code,"  U10 = CD[0]*(Data->twozeta_c*Data->twozeta_d*Data->oo2n);\n");
  fprintf(code,"  U11 = CD[1]*(Data->twozeta_c*Data->twozeta_d*Data->oo2n);\n");
  fprintf(code,"  U12 = CD[2]*(Data->twozeta_c*Data->twozeta_d*Data->oo2n);\n");
  fprintf(code,"  U0  = (Data->twozeta_c - Data->twozeta_d*(Data->twozeta_c*CDdotCA + %lf))*Data->oo2n;\n",(double)(lc+1));
