...
int emit_vrr_t2_build(int old_am, int new_am, int max_class_size)
{
  ...
  for(la=0;la<=new_am;la++) {
	...
	for(f=0;f<num_subfunctions;f++)
	  fprintf(vrr_header, "double *_t2_build_%c0%c0_%d(prim_data *, contr_data *, double *, const double *, const double *, const double *, const double *, const double *);\n",
		  am_letter[la],am_letter[lc],f);
      }
      else {
	split = 0;
	fprintf(vrr_header," _t2_build_%c0%c0(Data,ShellData,vp,i0,i1,i2,i3,i4);}\n",am_letter[la],am_letter[lc]);
	fprintf(vrr_header,"void _t2_build_%c0%c0(prim_data *, contr_data *, double *, const double *, const double *, const double *, const double *, const double *);\n",
		am_letter[la],am_letter[lc]);
      }
