...
int emit_grt_order(int old_am, int new_am)
{
  ...
  for(la=0;la<=new_am;la++) {
	...
	Write the overhead to the HRR code
      ...
      fprintf(hrr_code,"extern void %s();\n\n",vrr_function_name);
	...
	Include the function into the hrr_header.h and init_libint.c
       -------------------------------------------------------------*/
      fprintf(hrr_header,"void %s(int);\n",hrr_function_name);
      fprintf(init_code,"  build_grt_abcd[%d][%d][%d][%d] = %s;\n",la-lb,lb,lc-ld,ld,hrr_function_name);
	...
	Declare local variables
       ------------------------*/
      fprintf(hrr_code," extern double *int_stack;\n extern double *te_ptr[4];\n");
      fprintf(hrr_code," extern prim_data *Shell_Data;\n\n");
      fprintf(hrr_code," extern double *r12vrr_stack;\n extern prim_data *Data;\n");
      fprintf(hrr_code," extern double *gvrr_classes[%d][%d];\n extern double *rvrr_classes[%d][%d];\n",
	      new_am+1,new_am+1,new_am+1,new_am+1);
      fprintf(hrr_code," extern double *t1vrr_classes[%d][%d];\n extern double *t2vrr_classes[%d][%d];\n\n",
	      new_am+1,new_am+1,new_am+1,new_am+1);
	...
	if (hrr_nodes[i].B == 0 && hrr_nodes[i].D == 0) {
      ...
      fprintf(hrr_code," r12vrr_stack = int_stack + %d;\n",base_mem);
	...
	if (hrr_nodes[j].B > 0 || hrr_nodes[j].D > 0) {
	  ...
	  if (hrr_nodes[j].B == 0 && hrr_nodes[j].D != 0) {
	    if (hrr_nodes[j].grt_type == 3)
	      fprintf(hrr_code, "   t2hrr3_build_%c%c(int_stack+%d,",
	    ...
	    if (hrr_nodes[j].grt_type == 2)
	      fprintf(hrr_code, "   t1hrr1_build_%c%c(int_stack+%d,",
	...
	max_stack_size = vrr_mem;
      fprintf(vrr_code," extern double *r12vrr_stack;\n extern prim_data *Data;\n double *tmp, *target_ptr;\n int i;\n");
      fprintf(vrr_code," extern double *gvrr_classes[%d][%d];\n extern double *rvrr_classes[%d][%d];\n",
	      new_am+1,new_am+1,new_am+1,new_am+1);
      fprintf(vrr_code," extern double *t1vrr_classes[%d][%d];\n extern double *t2vrr_classes[%d][%d];\n\n",
	      new_am+1,new_am+1,new_am+1,new_am+1);
