...
int emit_vrr_build(int old_am, int new_am, int max_class_size)
{
  FILE *code;
  int i, j, k, l, f;
  int a, b;
  ...
  for(la=0;la<=new_am;la++) {
	...
	k1max = la - 1;
      ...
      fprintf(vrr_header,"#define _BUILD_%c0%c0(vp,i0,i1,i2,i3,i4) {",am_letter[la],am_letter[lc]);
	...
	fprintf(vrr_header," tmp = _build_%c0%c0_0(vp,i0,i1,i2,i3,i4); \\\n",am_letter[la],am_letter[lc]);
	for(f=1;f<num_subfunctions;f++)
	  fprintf(vrr_header," tmp = _build_%c0%c0_%d(tmp,i0,i1,i2,i3,i4); \\\n",am_letter[la],am_letter[lc],f);
	fprintf(vrr_header,"}\n");
	for(f=0;f<num_subfunctions;f++)
	  fprintf(vrr_header,
   "double *_build_%c0%c0_%d(double *, const double *, const double *, const double *, const double *, const double *);\n",
		  am_letter[la],am_letter[lc],f);
      }
      else {
	split = 0;
	fprintf(vrr_header," _build_%c0%c0(vp,i0,i1,i2,i3,i4);}\n",am_letter[la],am_letter[lc]);
	fprintf(vrr_header,"void _build_%c0%c0(double *, const double *, const double *, const double *, const double *, const double *);\n",
		am_letter[la],am_letter[lc]);
      }
      ...
      sprintf(code_name,"build_%c0%c0.c",am_letter[la],am_letter[lc]);
