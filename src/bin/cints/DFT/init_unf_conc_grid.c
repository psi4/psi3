...
void init_uniform_con_grid(void){
	...
	/* ---- This is the only part that depends on whether 
	   you it is a pruned grid or not */
	grid->atomic_grid[i].chunk_num = grid->prim_atomic_grid.chunk_num;
	...
	for(j=0;j<grid->atomic_grid[i].chunk_num;j++){
	    ...
	    prim_chnk = &(grid->prim_atomic_grid.leb_chunk[j]);
