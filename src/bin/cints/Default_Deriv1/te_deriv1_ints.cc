...
void te_deriv1_ints()
{
  ...
  vector< vector< vector<cdsalc_elem> > > plist_salcs;
  ...
  for (int usii=0; usii<Symmetry.num_unique_shells; usii++)
	...
	for (int usll=0; usll<=usll_max; usll++){
          ...
          plist_salcs.clear();
          plist_salcs.resize(num_unique_quartets);
...
#define NO_PERM_SYMM 0
		...
		for (pk = 0; pk < np_k; pk++) {
		  ...
		  for (pl = 0; pl < max_pl; pl++){
            ...
            std::vector<int> nuc;
            nuc.push_back(center_i);
            nuc.push_back(center_j);
            nuc.push_back(center_k);
            nuc.push_back(center_l);
            ...
            for(int uc=0; uc<num_unique_centers; uc++) {
              ...
              for(int xyz=0; xyz<3; xyz++, totderindex++, uniquetotderindex++) {
                ...
                const double* data = norm_quartet(cart_ints[totderindex], target_ints, orig_am, BasisSet.puream);
                if (data != target_ints)
            ...
            //
            vector< vector<cdsalc_elem> >& plist_salc = plist_salcs[plquartet];
            plist_salc.clear();
            plist_salc.resize(Symmetry.nirreps);
            for(int uc=0; uc<num_unique_centers; uc++) {
              ...
              for(int xyz=0; xyz<3; xyz++,abs_cart_der++,cart_der++) {
                ...
                for(int salc=0; salc<nsalcs; salc++) {
                  ...
                  plist_salc[irrep].push_back(elem);
                }
              }
            }
                  ...
                  for(ij=0;ij<npi_ij;ij++) { /*--- Loop over SO pairs from usij ---*/
                    ...
                    for(kl=0;kl<npi_kl;kl++) { /*--- Loop over SO pairs from uskl ---*/
                      ...
                      for(s=0;s<num_unique_quartets;s++){  /*--- Sum over petite list quartets ---*/
                        ...
                        const int ncontribs = plist_salcs[s][irrep_deriv].size();
                        for(int derop=0; derop<ncontribs; derop++) {
                          const cdsalc_elem& elem = plist_salcs[s][irrep_deriv][derop];
