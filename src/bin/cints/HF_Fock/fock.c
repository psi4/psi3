...
void fock()
{
  ...
  if (Symmetry.nirreps > 1)
    init_htable(Symmetry.max_stab_index);
  schwartz_eri();
 ...
 -------------------------------------------------*/
  ...
  /*--- Decide what unique shell quartets out of (ij|kl), (ik|jl), and (il|jk) are unique
		...
		for(dcr_ijkl=0;dcr_ijkl<Symmetry.dcr_dim[stab_ij][stab_kl];dcr_ijkl++){
		  ...
		  for(dcr_kl=0;dcr_kl<Symmetry.dcr_dim[stab_k][stab_l];dcr_kl++) {
		    ...
		    if(!(total_am%2)||
		      ...
		      new_quartet = put_entry(key1,si,sj,sk,sl,q4ijkl,0,0);
		      if (new_quartet > -1) {
			...
			new_quartet = put_entry(key2,si,sk,sj,sl,0,q4ijkl,0);
			if (new_quartet > -1) {
			  key_arr[count] = new_quartet;
			  count++;
			}
			...
			new_quartet = put_entry(key3,si,sl,sk,sj,0,0,q4ijkl);
			if (new_quartet > -1) {
			  key_arr[count] = new_quartet;
			  count++;
			}
	  ...
	  num_unique_quartets = count;
		...
		htable[htable_ptr].key = EMPTY_KEY;
		si = htable[htable_ptr].si;
		sj = htable[htable_ptr].sj;
		sk = htable[htable_ptr].sk;
		sl = htable[htable_ptr].sl;
		dmax = MAX(BasisSet.shell_pairs[si][sj].Dmax, BasisSet.shell_pairs[sk][sl].Dmax);
		dmax = MAX(dmax, 0.25*BasisSet.shell_pairs[si][sk].Dmax);
		dmax = MAX(dmax, 0.25*BasisSet.shell_pairs[si][sl].Dmax);
		dmax = MAX(dmax, 0.25*BasisSet.shell_pairs[sj][sk].Dmax);
		...
		fac1 = htable[htable_ptr].q4ijkl;
		fac2 = htable[htable_ptr].q4ikjl;
		fac3 = htable[htable_ptr].q4ilkj;
		if (si == sj && si == sk ||
		    sj == sk && sj == sl ||
		    si == sj && si == sl ||
		    si == sk && si == sl) {
		...
		for(n=0;n<num;n++) {
		  ...
		  if (ii == jj && ii == kk ||
		      ...
		      ii == jj && ii == ll ||
		      ii == kk && ii == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c1*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c1*ffac1);
		  }
		  else if (ii == kk || jj == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c2*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c2*ffac1);
		      Gskel[si][sk][i][k] += sp_jl->dmat[j][l]*(c3*ffac2);
		      Gskel[sj][sl][j][l] += sp_ik->dmat[i][k]*(c3*ffac2);
		  }
		  else if (jj == kk || ii == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c2*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c2*ffac1);
		      Gskel[si][sl][i][l] += sp_jk->dmat[j][k]*(c3*ffac3);
		      Gskel[sj][sk][j][k] += sp_il->dmat[i][l]*(c3*ffac3);
		  }
		  else if (ii == jj || kk == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(ffac1);
		      Gskel[si][sk][i][k] += sp_jl->dmat[j][l]*(c4*ffac2);
		      Gskel[sj][sl][j][l] += sp_ik->dmat[i][k]*(c4*ffac2);
		  }
		  else {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(ffac1);
		      Gskel[si][sk][i][k] += sp_jl->dmat[j][l]*(c4*ffac2);
		      Gskel[sj][sl][j][l] += sp_ik->dmat[i][k]*(c4*ffac2);
		      Gskel[si][sl][i][l] += sp_jk->dmat[j][k]*(c4*ffac3);
		      Gskel[sj][sk][j][k] += sp_il->dmat[i][l]*(c4*ffac3);
		  }
		}
	      }
		...
		for(n=0;n<num;n++) {
		  ...
		  if (ii == jj && ii == kk ||
		      ...
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c1*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c1*ffac1);
		      Gskel_o[si][sj][i][j] += sp_kl->dmato[k][l]*(c1*ffac1);
		      Gskel_o[sk][sl][k][l] += sp_ij->dmato[i][j]*(c1*ffac1);
		  }
		  else if (ii == kk || jj == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c2*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c2*ffac1);
		      Gskel[si][sk][i][k] += sp_jl->dmat[j][l]*(c3*ffac2);
		      Gskel[sj][sl][j][l] += sp_ik->dmat[i][k]*(c3*ffac2);
		      Gskel_o[si][sj][i][j] -= sp_kl->dmato[k][l]*(c4*ffac1);
		      Gskel_o[sk][sl][k][l] -= sp_ij->dmato[i][j]*(c4*ffac1);
		      Gskel_o[si][sk][i][k] -= sp_jl->dmato[j][l]*(c3*ffac2);
		      Gskel_o[sj][sl][j][l] -= sp_ik->dmato[i][k]*(c3*ffac2);
		  }
		  else if (jj == kk || ii == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*(c2*ffac1);
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*(c2*ffac1);
		      ...
		      Gskel_o[si][sj][i][j] -= sp_kl->dmato[k][l]*(c4*ffac1);
		      Gskel_o[sk][sl][k][l] -= sp_ij->dmato[i][j]*(c4*ffac1);
		      Gskel_o[si][sl][i][l] -= sp_jk->dmato[j][k]*(c3*ffac3);
		      Gskel_o[sj][sk][j][k] -= sp_il->dmato[i][l]*(c3*ffac3);
		  }
		  else if (ii == jj || kk == ll) {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*ffac1;
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*ffac1;
		      Gskel[si][sk][i][k] += sp_jl->dmat[j][l]*(c4*ffac2);
		      Gskel[sj][sl][j][l] += sp_ik->dmat[i][k]*(c4*ffac2);
		      Gskel_o[si][sk][i][k] -= sp_jl->dmato[j][l]*(c4*ffac2);
		      Gskel_o[sj][sl][j][l] -= sp_ik->dmato[i][k]*(c4*ffac2);
		  }
		  else {
		      Gskel[si][sj][i][j] += sp_kl->dmat[k][l]*ffac1;
		      Gskel[sk][sl][k][l] += sp_ij->dmat[i][j]*ffac1;
		      ...
		      Gskel_o[si][sk][i][k] -= sp_jl->dmato[j][l]*(c4*ffac2);
		      Gskel_o[sj][sl][j][l] -= sp_ik->dmato[i][k]*(c4*ffac2);
		      Gskel_o[si][sl][i][l] -= sp_jk->dmato[j][k]*(c4*ffac3);
		      Gskel_o[sj][sk][j][k] -= sp_il->dmato[i][l]*(c4*ffac3);
		  }
		}
	      }
