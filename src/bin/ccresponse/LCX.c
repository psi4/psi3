...
double LCX(char *cart, int irrep, double omega)
{
  ...
  sprintf(lbl, "MuBAR_%1s_EiAb", cart);
  dpd_buf4_init(&mu2, CC_LR, irrep, 11, 5, 11, 5, 0, lbl);
  ...
  dpd_buf4_init(&Z, CC_TMP1, 0, 0, 5, 0, 5, 0, "Z(Ij,Ab)");
  sprintf(lbl, "MuBAR_%1s_MbIj", cart);
  dpd_buf4_init(&mu2, CC_LR, irrep, 10, 0, 10, 0, 0, lbl);
