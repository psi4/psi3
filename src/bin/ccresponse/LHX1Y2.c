...
double LHX1Y2(char *cart, int irrep, double omega)
{
  ...
  dpd_file2_init(&z1, CC_TMP0, irrep, 1, 1, lbl);
  dpd_buf4_init(&I, CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
  sprintf(lbl, "X_%1s_IjAb (%5.3f)", cart, -omega);
  dpd_buf4_init(&Y2, CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
  ...
  sprintf(lbl, "Z_%1s_ME", cart);
  dpd_file2_init(&z1, CC_TMP0, irrep, 0, 1, lbl);
  sprintf(lbl, "X_%1s_IA (%5.3f)", cart, omega);
  dpd_file2_init(&X1, CC_OEI, irrep, 0, 1, lbl);
  dpd_buf4_init(&I, CC_DINTS, 0, 0, 5, 0, 5, 0, "D 2<ij|ab> - <ij|ba>");
  ...
  sprintf(lbl, "Z_%1s_MbEj (Mb,jE)", cart);
  ...
  dpd_buf4_close(&W);
  sprintf(lbl, "Z_%1s_MbEj (ME,jb)", cart);
  dpd_buf4_sort(&Z1, CC_TMP0, psrq, 10, 10, lbl);
  dpd_buf4_close(&Z1);

  sprintf(lbl, "Z_%1s_WMbEj (bM,Ej)", cart);
  dpd_buf4_init(&Z1, CC_TMP0, irrep, 11, 11, 11, 11, 0, lbl);
  ...
  sprintf(lbl, "Z_%1s_MbeJ (Mb,eJ)", cart);
  dpd_buf4_init(&Z1, CC_TMP0, irrep, 10, 11, 10, 11, 0, lbl);
  dpd_buf4_init(&W, CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe");
  ...
  sprintf(lbl, "Z_%1s_MbEj (ME,jb)", cart);
  dpd_buf4_init(&Z1, CC_TMP0, irrep, 10, 10, 10, 10, 0, lbl);
  sprintf(lbl, "Z_%1s_(2MbEj+MbeJ) (ME,JB)", cart);
  ...
  sprintf(lbl, "X_%1s_IA (%5.3f)", cart, omega);
  dpd_file2_init(&X1, CC_OEI, irrep, 0, 1, lbl);
  sprintf(lbl, "Z_%1s_MnjI", cart);
  dpd_buf4_init(&Z1, CC_TMP0, irrep, 0, 0, 0, 0, 0, lbl);
  dpd_buf4_init(&W, CC_HBAR, 0, 0, 11, 0, 11, 0, "WMnIe");
  dpd_contract244(&X1, &W, &Z1, 1, 2, 1, 1, 0);
  dpd_buf4_close(&W);
  sprintf(lbl, "Z_%1s_MnIj", cart);
  ...
  sprintf(lbl, "Z_%1s_AmIj (mA,Ij)", cart);
  dpd_buf4_init(&Z1, CC_TMP0, irrep, 10, 0, 10, 0, 0, lbl);
  sprintf(lbl, "X_%1s_IjAb (%5.3f)", cart, -omega);
  dpd_buf4_init(&Y2, CC_LR, irrep, 0, 5, 0, 5, 0, lbl);
  dpd_buf4_init(&W, CC_HBAR, 0, 10, 5, 10, 5, 0, "WAmEf");
