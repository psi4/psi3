...
void hbar_extra(void) {
  ...
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 7, 10, 7, 0, "WAMEF");
  dpd_buf4_sort(&W, CC_HBAR, qprs, 11, 7, "WAMEF (AM,E>F)");
  dpd_buf4_close(&W);
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 7, 10, 7, 0, "Wamef");
  dpd_buf4_sort(&W, CC_HBAR, qprs, 11, 7, "Wamef (am,e>f)");
  dpd_buf4_close(&W);
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 5, 10, 5, 0, "WAmEf");
  ...
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 0, 10, 0, 0, "WmBiJ");
  dpd_buf4_sort(&W, CC_HBAR, pqsr, 10, 0, "WmBiJ (mB,Ji)");
  dpd_buf4_close(&W);
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 0, 10, 0, 0, "WmBiJ (mB,Ji)");
  dpd_buf4_sort(&W, CC_HBAR, qprs, 11, 0, "WmBiJ (Bm,Ji)");
  dpd_buf4_close(&W);

  dpd_buf4_init(&W, CC_HBAR, irrep, 11, 5, 11, 5, 0, "WeIaB");
  dpd_buf4_sort(&W, CC_HBAR, qprs, 10, 5, "WeIaB (Ie,aB)");
  dpd_buf4_close(&W);
  dpd_buf4_init(&W, CC_HBAR, irrep, 10, 5, 10, 5, 0, "WeIaB (Ie,aB)");
