...
void WamefSD(int i, int irrep) {
 ...
 /* SIA += 0.5 CIMEF* WAMEF + CImEf * WAmEf */
  dpd_buf4_init(&WAMEF, CC_HBAR, irrep, 11, 7, 11, 7, 0, "WAMEF (AM,E>F)");
  sprintf(lbl, "%s %d", "CMNEF", i);
  dpd_buf4_init(&CMNEF, EOM_CMNEF, irrep, 0, 7, 2, 7, 0, lbl);
  dpd_contract442(&CMNEF, &WAMEF, &SIA, 0, 0, 1.0, 1.0);
  dpd_buf4_close(&CMNEF);
  dpd_buf4_close(&WAMEF);

  dpd_buf4_init(&WAmEf, CC_HBAR, irrep, 11, 5, 11, 5, 0, "WAmEf (Am,Ef)");
  sprintf(lbl, "%s %d", "CMnEf", i);
