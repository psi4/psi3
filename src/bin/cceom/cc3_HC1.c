...
void HC1_Wamef(int i, int C_irr) {
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_close(&WAMEF);  
    dpd_buf4_init(&WAMEF, CC3_HC1, 0, 20, 7, 20, 7, 0, "HC1 WAMEF (MA,F>E)");
    ...
    dpd_buf4_close(&Wamef); 
    dpd_buf4_init(&Wamef, CC3_HC1, 0, 30, 17, 30, 17, 0, "HC1 Wamef (ma,f>e)");
    dpd_buf4_scm(&Wamef, -1.0);
    dpd_buf4_close(&Wamef);  
...
void HC1_Wmbij(int i, int C_irr)
{
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_init(&Z2, CC_TMP0, 0, 30, 10, 30, 10, 0, "Z1(mb,ij)");
...
void HC1_Wabei(int i, int C_irr) { 
  ...
  if(params.ref == 0) { /** RHF **/
    ...
    dpd_buf4_init(&Z1, CC_TMP1, 0, 5, 21, 5, 21, 0, "Z (AB,EI)");
    ...
    dpd_buf4_init(&Z1, CC_TMP1, 0, 15, 31, 15, 31, 0, "Z (ab,ei)");
    ...
    dpd_buf4_init(&W, CC3_HC1, 0, 20, 7, 20, 7, 0, "HC1 WABEI (IE,B>A)");
    dpd_buf4_scm(&W, -1.0);
    dpd_buf4_close(&W);
    ...
    dpd_buf4_init(&W, CC3_HC1, 0, 30, 17, 30, 17, 0, "HC1 Wabei (ie,b>a)");
    dpd_buf4_scm(&W, -1.0);
    dpd_buf4_close(&W);
