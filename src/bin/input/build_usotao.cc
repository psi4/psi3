  ...
  double *tmp_ptr;
	...
	for(irr=0;irr<nirreps;irr++) {  /* ... count number of cartesian SOs in each irreps generated by the unique atoms and its children */
	  ...
	  if (puream) {                 /* Do the same for pureang SOs*/
	    for(shell=shell_first;shell<shell_last;shell++)
	      pureang_cnt += num_pureang_so_in_class[clss][shell_ang_mom[shell]][irr];
	...
	for(irr=0;irr<nirreps;irr++)    /*Go over each symmetry block*/
	  if (num_cart_so_in_class[clss][l][irr] != 0) { /*If there are SO's of this irrep from this shell - proceed*/
		...
		if (class_so_coeff[clss][l][cnt][ao] != 0.0) {   
		  usotao_cart[cart_so_num_in_symblk][ao_offset+ao] = class_so_coeff[clss][l][cnt][ao]; /*... and add it to the usotao_cart*/
		  ...
		  if (puream)
			...
			if (flag == 0) {                                     /*****If it has not been added yet ...*/
			  ...
			  usotbf[pureang_so_num_in_symblk+num_so_added][first_basisfn_shell[shell]+m+l] = class_so_coeff[clss][l][cnt][ao];
			  num_so_added++;
			}
	    ...
	    if (puream)
	      pureang_symblk_offset[irr] += num_pureang_so_in_class[clss][l][irr];  /* ... and pureang SO's*/
	  }
      }
    }
	...
	for(irr=0;irr<nirreps;irr++) {
	  ...
	  for(shell=shell_first;shell<shell_last;shell++) {
	    num_so_from_shell = num_pureang_so_in_class[clss][shell_ang_mom[shell]][irr];
