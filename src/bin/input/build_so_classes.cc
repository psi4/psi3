...
void build_so_classes()
{
  ...
  int *first_basisfn_in_symblk;
	...
	max_angmom_class[clss] = (max_angmom_class[clss] > shell_ang_mom[j]) ? max_angmom_class[clss] : shell_ang_mom[j];
    ...
    for(clss=0;clss<num_classes;clss++)
      num_cart_so_in_class[clss] = init_int_matrix(max_angmom_class[clss]+1,nirreps);
    ...
    if (puream) {
      ...
      for(clss=0;clss<num_classes;clss++)
	num_pureang_so_in_class[clss] = init_int_matrix(max_angmom_class[clss]+1,nirreps);
    }
	...
	if (print_lvl >= DEBUGPRINT) {
	  fprintf(outfile,"    Class = %d  l = %d\n     Number of cartesian SOs in symmetry blocks:",clss+1,l);
	  for(irr=0;irr<nirreps;irr++)
	    fprintf(outfile," %d",num_cart_so_in_class[clss][l][irr]);
	  fprintf(outfile,"\n\n");
	  if (puream) {
	    fprintf(outfile,"    Class = %d  l = %d\n     Number of pure angular momentum SOs in symmetry blocks:",clss+1,l);
	    for(irr=0;irr<nirreps;irr++)
