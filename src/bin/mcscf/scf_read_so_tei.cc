...
void SCF::read_so_tei_form_PK()
{
  ...
  do{
    ...
    for(index=0;index<nbuf;index++){
      ...
      if(pair_sym[p][q] == 0)
      {
        PK[INDEX(pair[p][q],pair[r][s])]    += value;
      }
      if(pair_sym[p][r] == 0)
      {
        if((p == r) || (q == s))
          PK[INDEX(pair[p][r],pair[q][s])] -= 0.5 * value;
        else
          PK[INDEX(pair[p][r],pair[q][s])] -= 0.25 * value;
      }
      if(pair_sym[p][s] == 0){
        if((p != q) && (r != s)){
          if((p == s) || (q == r))
            PK[INDEX(pair[p][s],pair[q][r])] -= 0.5 * value;
          else
            PK[INDEX(pair[p][s],pair[q][r])] -= 0.25 * value;
        }
      }

#if STORE_TEI
      tei_so[pair_sym[p][q]][INDEX(pair[p][q],pair[r][s])] = value;
#endif
  ...
  for(int pq = 0; pq < pairpi[0]; ++pq){
    PK[INDEX(pq,pq)] *= 0.5;
  }
  fprintf(outfile,"done.");
  fprintf(outfile,"\n  SCF read %d non-zero integrals",elements);
  fflush(outfile);
}
  ...
  do{
    ...
    for(index=0;index<nbuf;index++){
      ...
      if(pair_sym[p][r] == 0)
      {
        if((p == r) || (q == s)){
          PK[INDEX(pair[p][r],pair[q][s])] -= 0.5 * value;
           K[INDEX(pair[p][r],pair[q][s])] -= 0.5 * value;
        }
        else{
          PK[INDEX(pair[p][r],pair[q][s])] -= 0.25 * value;
           K[INDEX(pair[p][r],pair[q][s])] -= 0.25 * value;
        }
      }
      if(pair_sym[p][s] == 0){
        if((p != q) && (r != s)){
          if((p == s) || (q == r)){
            PK[INDEX(pair[p][s],pair[q][r])] -= 0.5 * value;
             K[INDEX(pair[p][s],pair[q][r])] -= 0.5 * value;
          }
          else{
            PK[INDEX(pair[p][s],pair[q][r])] -= 0.25 * value;
             K[INDEX(pair[p][s],pair[q][r])] -= 0.25 * value;
          }
        }
      }
