...
void MRCCSD_T::compute_restricted()
{
  ...
  fprintf(outfile,"\n\n  Using restricted loops algorithm\n");
...
//  size_t ntriplets = ijk;
  ...
  while(++ijk){
    ...
    if((i_abs < j_abs) and (j_abs < k_abs)){
          ...
          if(is_aocc[mu][i_abs] && is_aocc[mu][j_abs] && is_aocc[mu][k_abs]){
            ...
            while(++abc){
              ...
              if(is_avir_mu[a_abs] && is_avir_mu[b_abs] && is_avir_mu[c_abs]){
                ...
                // Compute the energy
                e4T[mu] += W[mu][ijk.sym]->get(a_sym,a_rel,bc_rel) * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel) / 6.0;
                if((i_sym == a_sym) & (jk_sym == bc_sym)){
                  e4ST[mu] += 0.5 * T1_ov_mu[i_sym][i_rel][a_rel] * V_oovv[jk_sym][jk_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] += 0.5 *  F_ov_mu[i_sym][i_rel][a_rel] * T2_oovv_mu[jk_sym][jk_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
                if((j_sym == a_sym) & (ik_sym == bc_sym)){
                  e4ST[mu] -= 0.5 * T1_ov_mu[j_sym][j_rel][a_rel] * V_oovv[ik_sym][ik_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] -= 0.5 *  F_ov_mu[j_sym][j_rel][a_rel] * T2_oovv_mu[ik_sym][ik_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
                if((k_sym == a_sym) & (ji_sym == bc_sym)){
                  e4ST[mu] -= 0.5 * T1_ov_mu[k_sym][k_rel][a_rel] * V_oovv[ji_sym][ji_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] -= 0.5 *  F_ov_mu[k_sym][k_rel][a_rel] * T2_oovv_mu[ji_sym][ji_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
              }
            }  // End loop over abc
      ...
      for(int mu = 0; mu < nrefs; ++mu){
        compute_ooo_contribution_to_Heff_restricted(i_abs,j_abs,k_abs,mu,T[mu][ijk.sym]);
      }
          ...
          if(is_aocc[mu][i_abs] && is_aocc[mu][j_abs] && is_bocc[mu][k_abs]){
            ...
            while(++abc){
              ...
              if(is_avir_mu[a_abs] && is_avir_mu[b_abs] && is_bvir_mu[c_abs]){
                ...
                // Compute the energy
                e4T[mu] += W[mu][ijk.sym]->get(a_sym,a_rel,bc_rel) * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel) / 2.0;
                if((i_sym == a_sym) & (jk_sym == bc_sym)){
                ...
                if((j_sym == a_sym) & (ik_sym == bc_sym)){
                  e4ST[mu] -= T1_ov_mu[j_sym][j_rel][a_rel] *  V_oOvV[ik_sym][ik_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] -=  F_ov_mu[j_sym][j_rel][a_rel] * T2_oOvV_mu[ik_sym][ik_rel][bc_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
                if((k_sym == c_sym) & (ij_sym == ab_sym)){
                  e4ST[mu] += 0.5 * T1_OV_mu[k_sym][k_rel][c_rel] *  V_oovv[ij_sym][ij_rel][ab_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] += 0.5 *  F_OV_mu[k_sym][k_rel][c_rel] * T2_oovv_mu[ij_sym][ij_rel][ab_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
              }
            }  // End loop over abc
            newE += abs(e4T[mu]) + abs(e4ST[mu]) + abs(e4DT[mu]);
            ...
            while(++abc){
              ...
              if(is_avir_mu[a_abs] && is_bvir_mu[b_abs] && is_bvir_mu[c_abs]){
                ...
                if((j_sym == c_sym) & (ik_sym == ab_sym)){
                  e4ST[mu] -= T1_OV_mu[j_sym][j_rel][c_rel] *  V_oOvV[ik_sym][ik_rel][ab_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                  e4DT[mu] -=  F_OV_mu[j_sym][j_rel][c_rel] * T2_oOvV_mu[ik_sym][ik_rel][ab_rel] * T[mu][ijk.sym]->get(a_sym,a_rel,bc_rel);
                }
              }
            }  // End loop over abc
            newE += abs(e4T[mu]) + abs(e4ST[mu]) + abs(e4DT[mu]);
