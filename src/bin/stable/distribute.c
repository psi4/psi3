...
void distribute(void)
{
  double tolerance;
  struct iwlbuf InBuf;
  struct iwlbuf ABuf, CBuf, DBuf, EBuf;
  int lastbuf;
  Value *valptr;
  Label *lblptr;
  int idx, p, q, r, s;
  ...
  iwl_buf_init(&CBuf, 91, tolerance, 0, 0);
  iwl_buf_init(&DBuf, 92, tolerance, 0, 0);
  iwl_buf_init(&EBuf, 93, tolerance, 0, 0);
  ...
  for (idx=4*InBuf.idx; InBuf.idx < InBuf.inbuf; InBuf.idx++) {
    ...
    classify(p,q,r,s,value,&ABuf,&CBuf,&DBuf,&EBuf);
