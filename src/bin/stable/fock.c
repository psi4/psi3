...
void fock(void)
{
  ...
  dpd_buf4_init(&AInts_anti, PSIF_MO_HESS, 0, 0, 0, 0, 0, 1, "A <ij|kl>");
  dpd_buf4_init(&AInts, PSIF_MO_HESS, 0, 0, 0, 0, 0, 0, "A <ij|kl>");
  ...
  dpd_buf4_copy(&C, PSIF_MO_HESS, "C <ia||jb>");
  dpd_buf4_close(&C);
  dpd_buf4_init(&D, PSIF_MO_HESS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_buf4_sort(&D, PSIF_MO_HESS, psqr, 10, 10, "D <ij|ab> (ib,ja)");
  dpd_buf4_close(&D);
  dpd_buf4_init(&D, PSIF_MO_HESS, 0, 10, 10, 10, 10, 0, "D <ij|ab> (ib,ja)");
  dpd_buf4_init(&C, PSIF_MO_HESS, 0, 10, 10, 10, 10, 0, "C <ia||jb>");
  dpd_buf4_axpy(&D, &C, -1);
  dpd_buf4_close(&D);
  dpd_buf4_close(&C);
  
  dpd_buf4_init(&CInts_anti, PSIF_MO_HESS, 0, 10, 10, 10, 10, 0, "C <ia||jb>");
  dpd_buf4_init(&CInts, PSIF_MO_HESS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
  ...
  dpd_buf4_init(&EInts_anti, PSIF_MO_HESS, 0, 11, 0, 11, 0, 1, "E <ai|jk>");
  dpd_buf4_init(&EInts, PSIF_MO_HESS, 0, 11, 0, 11, 0, 0, "E <ai|jk>");
