...
/* BUILD_A(): Construct the RHF/ROHF molecular orbital Hessian, A. At
** the moment we're actually building all symmetry blocks of A, though
** for the orbital Z-vector equations we really only need the totally
...
void build_A(void)
{
  ...
  dpd_buf4_init(&D, PSIF_MO_HESS, 0, 0, 5, 0, 5, 0, "D <ij|ab>");
  dpd_buf4_sort(&D, PSIF_MO_HESS, rpsq, 11, 11, "A(EM,AI)");
  dpd_buf4_close(&D);
  dpd_buf4_init(&Amat, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "A(EM,AI)");
  dpd_buf4_sort(&Amat, PSIF_MO_HESS, psrq, 11, 11, "D <im|ea> (ei,am)");
  dpd_buf4_scm(&Amat, 2.0);
  dpd_buf4_copy(&Amat, PSIF_MO_HESS, "A(EM,ai)");
  dpd_buf4_init(&D, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "D <im|ea> (ei,am)");
  dpd_buf4_axpy(&D, &Amat, -1.0);
  dpd_buf4_close(&D);
  dpd_buf4_init(&C, PSIF_MO_HESS, 0, 10, 10, 10, 10, 0, "C <ia|jb>");
  dpd_buf4_sort(&C, PSIF_MO_HESS, qpsr, 11, 11, "C <ai|bj>");
  dpd_buf4_close(&C);
  dpd_buf4_init(&C, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "C <ai|bj>");
  ...
  dpd_file2_init(&fIJ, PSIF_MO_HESS, 0, 0, 0, "fIJ");
  dpd_file2_mat_init(&fIJ);
  dpd_file2_mat_rd(&fIJ);
  dpd_file2_init(&fij, PSIF_MO_HESS, 0, 0, 0, "fij");
  dpd_file2_mat_init(&fij);
  dpd_file2_mat_rd(&fij);
  dpd_file2_init(&fAB, PSIF_MO_HESS, 0, 1, 1, "fAB");
  dpd_file2_mat_init(&fAB);
  dpd_file2_mat_rd(&fAB);
  dpd_file2_init(&fab, PSIF_MO_HESS, 0, 1, 1, "fab");
  dpd_file2_mat_init(&fab);
  dpd_file2_mat_rd(&fab);
  dpd_file2_init(&fIA, PSIF_MO_HESS, 0, 0, 1, "fIA");
  ...
  dpd_buf4_sort(&Amat, PSIF_MO_HESS, rspq, 11, 11, "A(em,AI)");
  dpd_buf4_close(&Amat);
  ...
  dpd_buf4_init(&Amat, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "A(EM,AI)");
  dpd_buf4_init(&Amat2, PSIF_MO_HESS, 0, 11, 11, 11, 11, 0, "A(em,ai)");
