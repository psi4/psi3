...
void sortmu(void)
{
  ...
  for(Gp=0; Gp < moinfo.nirreps; Gp++) { /* irrep of left-hand MO */
    ...
    for(p=0; p < moinfo.occpi[Gp]; p++) {
      P = moinfo.qt2pitz[moinfo.qt_occ[p+moinfo.occ_off[Gp]]];
      for(q=0; q < moinfo.occpi[Gq]; q++) {
	Q = moinfo.qt2pitz[moinfo.qt_occ[q+moinfo.occ_off[Gq]]];
  ...
  for(Gp=0; Gp < moinfo.nirreps; Gp++) { /* irrep of left-hand MO */
    ...
    for(p=0; p < moinfo.virtpi[Gp]; p++) {
      P = moinfo.qt2pitz[moinfo.qt_vir[p+moinfo.vir_off[Gp]]];
      for(q=0; q < moinfo.virtpi[Gq]; q++) {
	Q = moinfo.qt2pitz[moinfo.qt_vir[q+moinfo.vir_off[Gq]]];
	mu.matrix[Gp][p][q] = MU[P][Q];
      }
    }
  }
