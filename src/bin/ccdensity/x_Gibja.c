...
void x_Gibja(void)
{
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    dpd_buf4_init(&Z, EOM_TMP0, L_irr, 0, 11, 0, 11, 0, "Z(IM,AJ)");
    dpd_buf4_init(&L2, CC_GL, L_irr, 0, 5, 2, 7, 0, "LIJAB");
    dpd_file2_init(&T1A, CC_OEI, 0, 0, 1, "tIA");
    dpd_contract424(&L2, &T1A, &Z, 3, 1, 0, 1.0, 0.0);
    dpd_file2_close(&T1A);
    dpd_buf4_close(&L2);
    dpd_buf4_init(&Z1, EOM_TMP0, G_irr, 10, 11, 10, 11, 0, "Z(IB,AJ)");
    dpd_file2_init(&R1A, CC_GR, R_irr, 0, 1, "RIA");
    dpd_contract424(&Z, &R1A, &Z1, 1, 0, 1, 1.0, 0.0);
    dpd_file2_close(&R1A);
    dpd_buf4_close(&Z);
    dpd_buf4_sort(&Z1, EOM_TMP0, prqs, 10, 11, "Z(IA,BJ)");
    dpd_buf4_close(&Z1);
    dpd_buf4_init(&Z1, EOM_TMP0, G_irr, 10, 11, 10, 11, 0, "Z(IA,BJ)");
    dpd_buf4_sort(&Z1, EOM_TMP0, pqsr, 10, 10, "Z(IA,JB)");
    dpd_buf4_close(&Z1);
    dpd_buf4_init(&Z1, EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Z(IA,JB)");
    ...
    /* G(ia,jb) <-- - L(im,ae) T(j,e) R(m,b) */
    dpd_buf4_init(&Z, EOM_TMP0, 0, 0, 11, 0, 11, 0, "Z(im,aj)");
    dpd_buf4_init(&L2, CC_GL, 0, 0, 5, 2, 7, 0, "Lijab");
    dpd_file2_init(&T1B, CC_OEI, 0, 0, 1, "tia");
    dpd_contract424(&L2, &T1B, &Z, 3, 1, 0, 1.0, 0.0);
    dpd_file2_close(&T1B);
    dpd_buf4_close(&L2);
    dpd_buf4_init(&Z1, EOM_TMP0, 0, 10, 11, 10, 11, 0, "Z(ib,aj)");
    ...
    dpd_buf4_sort(&Z1, EOM_TMP0, prqs, 10, 11, "Z(ia,bj)");
    dpd_buf4_close(&Z1);
    dpd_buf4_init(&Z1, EOM_TMP0, 0, 10, 11, 10, 11, 0, "Z(ia,bj)");
    dpd_buf4_sort(&Z1, EOM_TMP0, pqsr, 10, 10, "Z(ia,jb)");
    dpd_buf4_close(&Z1);
    dpd_buf4_init(&Z1, EOM_TMP0, 0, 10, 10, 10, 10, 0, "Z(ia,jb)");
    ...
    /* G(iA,jB) <-- - (L2(iM,eA)*R(j,e)) * T(M,B) */
    dpd_buf4_init(&Z, EOM_TMP0, G_irr, 0, 11, 0, 11, 0, "Z(iM,Aj)");
    dpd_buf4_init(&L2, CC_GL, L_irr, 0, 5, 0, 5, 0, "LiJAb");
    dpd_file2_init(&R1A, CC_GR, R_irr, 0, 1, "Ria");
    ...
    /* symmetrize before adding ? */
    dpd_buf4_init(&G, EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "GIBJA");
    dpd_buf4_symm(&G);
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, EOM_TMP0, G_irr, 10, 10, 10, 10, 0, "Gibja");
    dpd_buf4_symm(&G);
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, EOM_TMP0, 0, 10, 10, 10, 10, 0, "GIbJa");
    dpd_buf4_symm(&G);
    dpd_buf4_close(&G);
    dpd_buf4_init(&G, EOM_TMP0, 0, 10, 10, 10, 10, 0, "GiBjA");
    dpd_buf4_symm(&G);
    dpd_buf4_close(&G);
    dpd_buf4_init(&GIbjA, EOM_TMP0, 0, 10, 10, 10, 10, 0, "GIbjA");
    dpd_buf4_init(&GiBJa, EOM_TMP0, 0, 10, 10, 10, 10, 0, "GiBJa");
    dpd_buf4_symm2(&GIbjA, &GiBJa);
    dpd_buf4_close(&GiBJa);
    dpd_buf4_sort(&GIbjA, EOM_TMP0, rspq, 10, 10, "GiBJa");
    dpd_buf4_close(&GIbjA);
