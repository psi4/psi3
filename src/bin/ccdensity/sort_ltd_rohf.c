...
void sort_ltd_rohf(struct RHO_Params S) 
{
  ...
  for(h=0; h < nirreps; h++) {
    for(i=0; i < occpi[h]; i++) {
      ...
      for(j=0; j < occpi[h^S.L_irr]; j++) {
        J = qt_occ[occ_off[h^S.L_irr] + j];
  ...
  for(h=0; h < nirreps; h++) {
    for(a=0; a < (virtpi[h] - openpi[h]); a++) {
      ...
      for(b=0; b < (virtpi[h^S.L_irr] - openpi[h^S.L_irr]); b++) {
        B = qt_vir[vir_off[h^S.L_irr] + b];
  ...
  for(h=0; h < nirreps; h++) {
    for(i=0; i < occpi[h]; i++) {
      I = qt_occ[occ_off[h] + i];
      for(a=0; a < (virtpi[h^S.L_irr] - openpi[h^S.L_irr]); a++) {
        A = qt_vir[vir_off[h^S.L_irr] + a];
  ...
  for(h=0; h < nirreps; h++) {
    for(i=0; i < (occpi[h] - openpi[h]); i++) { 
      I = qt_occ[occ_off[h] + i];
      for(j=0; j < (occpi[h^S.L_irr] - openpi[h^S.L_irr]); j++) {
        J = qt_occ[occ_off[h^S.L_irr] + j];
        moinfo.ltd[I][J] += D.matrix[h][i][j];
      }
    }
