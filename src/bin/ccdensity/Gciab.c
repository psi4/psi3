#include <stdio.h>
#include <dpd.h>
#define EXTERN
#include "globals.h"

void Gciab(void)
{
  int h, nirreps, a, b, c, i, A, B, C, I, Asym, Bsym, Csym, Isym, row, col;
  double value;
  struct oe_dpdfile L1, T1, g;
  struct dpdbuf G, L, T, Z, Z1, Z2, V;

  nirreps = moinfo.nirreps;

  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "GCIAB", 0, outfile);
  /* t(M,C) L(MI,AB) */
  dpd_buf_init(&L, CC_LAMPS, 0, 7, 2, 7, 0, "LIJAB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract212(&T1, &L, &G, 0, 0, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* l(M,C) Tau(MI,AB) */
  dpd_buf_init(&T, CC_TAMPS, 0, 7, 2, 7, 0, "tauIJAB", 0, outfile);
  dpd_oe_file_init(&L1, CC_OEI, 0, 1, "LIA", 0, outfile);
  dpd_contract212(&L1, &T, &G, 0, 0, 0, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&L1);
  dpd_buf_close(&T);
  dpd_buf_close(&G);
  /* t(I,E) L(MN,CE) --> Z(MN,CI) */
  dpd_buf_init(&Z, CC_TMP0, 2, 11, 2, 11, 0, "Z(MN,CI)", 0, outfile);
  dpd_buf_init(&L, CC_LAMPS, 2, 5, 2, 7, 0, "LIJAB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract221(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* -Z(MN,CI) Tau(MN,AB) --> G(CI,AB) */
  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "GCIAB", 0, outfile);
  dpd_buf_init(&T, CC_TAMPS, 2, 7, 2, 7, 0, "tauIJAB", 0, outfile);
  dpd_contract222(&Z, &T, &G, 1, 1, -1.0, 1.0, 0, outfile);
  dpd_buf_close(&T);
  dpd_buf_close(&Z);
  dpd_buf_close(&G);
  /* - V(IA,MC) T(M,B) --> Z(IA,BC) */
  dpd_buf_init(&Z, CC_TMP0, 10, 5, 10, 5, 0, "Z(IA,BC)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "VIAJB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, -1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP1, 10, 5, "Z(IC,BA)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(IC,BA)", 0, outfile);
  dpd_swap12(&Z, CC_TMP2, 11, 5, "Z(CI,BA)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 11, 5, 11, 5, 0, "Z(CI,BA)", 0, outfile);
  dpd_swap34(&Z, CC_TMP0, 11, 5, "Z(CI,AB)", 0, outfile);
  dpd_buf_init(&Z1, CC_TMP0, 11, 5, 11, 5, 0, "Z(CI,AB)", 0, outfile);
  dpd_axpy(&Z, &Z1, -1.0, 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 7, 0, "GCIAB", 0, outfile);
  dpd_axpy(&Z1, &G, 1.0, 0, outfile);
  dpd_buf_close(&Z1);
  dpd_buf_close(&G);

  /* - ( g(C,A) T(I,B) - g(C,B) T(I,A) ) --> G(CI,AB) */
  dpd_oe_file_init(&g, CC_OEI, 1, 1, "GAE", 0, outfile);
  dpd_oe_file_mat_init(&g);
  dpd_oe_file_mat_rd(&g, 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_oe_file_mat_init(&T1);
  dpd_oe_file_mat_rd(&T1, 0, outfile);

  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "GCIAB", 0, outfile);
  
  for(h=0; h < nirreps; h++) {
      dpd_buf_mat_irrep_init(&G, h);
      dpd_buf_mat_irrep_rd(&G, h, 0, outfile);

      for(row=0; row < G.params->rowtot[h]; row++) {
	  c = G.params->roworb[h][row][0];
	  i = G.params->roworb[h][row][1];
	  for(col=0; col < G.params->coltot[h]; col++) {
	      a = G.params->colorb[h][col][0];
	      b = G.params->colorb[h][col][1];

	      value = 0.0;

	      C = g.params->rowidx[c];  I = T1.params->rowidx[i];
	      Csym = g.params->psym[c]; Isym = T1.params->psym[i];
	      A = g.params->colidx[a];  B = T1.params->colidx[b];
	      Asym = g.params->qsym[a];  Bsym = T1.params->qsym[b];
	      
	      if((Csym==Asym) && (Isym==Bsym))
		  value += g.matrix[Csym][C][A] * T1.matrix[Isym][I][B];

	      B = g.params->colidx[b];  A = T1.params->colidx[a];
	      Bsym = g.params->qsym[b];  Asym = T1.params->qsym[a];
	      
	      if((Csym==Bsym) && (Isym==Asym))
		  value -= g.matrix[Csym][C][B] * T1.matrix[Isym][I][A];

	      G.matrix[h][row][col] -= value;
	    }
	}

      dpd_buf_mat_irrep_wrt(&G, h, 0, outfile);
      dpd_buf_mat_irrep_close(&G, h);
    }
  dpd_scm(&G, 0.5, 0, outfile);
  dpd_buf_close(&G);
  
  dpd_oe_file_mat_close(&g);
  dpd_oe_file_close(&g);
  dpd_oe_file_mat_close(&T1);
  dpd_oe_file_close(&T1);


  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "Gciab", 0, outfile);
  /* t(m,c) L(mi,ab) */
  dpd_buf_init(&L, CC_LAMPS, 0, 7, 2, 7, 0, "Lijab", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract212(&T1, &L, &G, 0, 0, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* l(m,c) Tau(mi,ab) */
  dpd_buf_init(&T, CC_TAMPS, 0, 7, 2, 7, 0, "tauijab", 0, outfile);
  dpd_oe_file_init(&L1, CC_OEI, 0, 1, "Lia", 0, outfile);
  dpd_contract212(&L1, &T, &G, 0, 0, 0, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&L1);
  dpd_buf_close(&T);
  dpd_buf_close(&G);
  /* t(i,e) L(mn,ce) --> Z(mn,ci) */
  dpd_buf_init(&Z, CC_TMP0, 2, 11, 2, 11, 0, "Z(mn,ci)", 0, outfile);
  dpd_buf_init(&L, CC_LAMPS, 2, 5, 2, 7, 0, "Lijab", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract221(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* -Z(mn,ci) Tau(mn,ab) --> G(ci,ab) */
  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "Gciab", 0, outfile);
  dpd_buf_init(&T, CC_TAMPS, 2, 7, 2, 7, 0, "tauijab", 0, outfile);
  dpd_contract222(&Z, &T, &G, 1, 1, -1.0, 1.0, 0, outfile);
  dpd_buf_close(&T);
  dpd_buf_close(&Z);
  dpd_buf_close(&G);
  /* - V(ia,mc) T(m,b) --> Z(ia,bc) */
  dpd_buf_init(&Z, CC_TMP0, 10, 5, 10, 5, 0, "Z(ia,bc)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "Viajb", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, -1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP1, 10, 5, "Z(ic,ba)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(ic,ba)", 0, outfile);
  dpd_swap12(&Z, CC_TMP2, 11, 5, "Z(ci,ba)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 11, 5, 11, 5, 0, "Z(ci,ba)", 0, outfile);
  dpd_swap34(&Z, CC_TMP0, 11, 5, "Z(ci,ab)", 0, outfile);
  dpd_buf_init(&Z1, CC_TMP0, 11, 5, 11, 5, 0, "Z(ci,ab)", 0, outfile);
  dpd_axpy(&Z, &Z1, -1.0, 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 7, 0, "Gciab", 0, outfile);
  dpd_axpy(&Z1, &G, 1.0, 0, outfile);
  dpd_buf_close(&Z1);
  dpd_buf_close(&G);

  /* - ( g(c,a) T(i,b) - g(c,b) T(i,a) ) --> G(ci,ab) */
  dpd_oe_file_init(&g, CC_OEI, 1, 1, "Gae", 0, outfile);
  dpd_oe_file_mat_init(&g);
  dpd_oe_file_mat_rd(&g, 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_oe_file_mat_init(&T1);
  dpd_oe_file_mat_rd(&T1, 0, outfile);

  dpd_buf_init(&G, CC_GAMMA, 11, 7, 11, 7, 0, "Gciab", 0, outfile);
  
  for(h=0; h < nirreps; h++) {
      dpd_buf_mat_irrep_init(&G, h);
      dpd_buf_mat_irrep_rd(&G, h, 0, outfile);

      for(row=0; row < G.params->rowtot[h]; row++) {
	  c = G.params->roworb[h][row][0];
	  i = G.params->roworb[h][row][1];
	  for(col=0; col < G.params->coltot[h]; col++) {
	      a = G.params->colorb[h][col][0];
	      b = G.params->colorb[h][col][1];

	      value = 0.0;

	      C = g.params->rowidx[c];  I = T1.params->rowidx[i];
	      Csym = g.params->psym[c]; Isym = T1.params->psym[i];
	      A = g.params->colidx[a];  B = T1.params->colidx[b];
	      Asym = g.params->qsym[a];  Bsym = T1.params->qsym[b];
	      
	      if((Csym==Asym) && (Isym==Bsym))
		  value += g.matrix[Csym][C][A] * T1.matrix[Isym][I][B];

	      B = g.params->colidx[b];  A = T1.params->colidx[a];
	      Bsym = g.params->qsym[b];  Asym = T1.params->qsym[a];
	      
	      if((Csym==Bsym) && (Isym==Asym))
		  value -= g.matrix[Csym][C][B] * T1.matrix[Isym][I][A];

	      G.matrix[h][row][col] -= value;
	    }
	}

      dpd_buf_mat_irrep_wrt(&G, h, 0, outfile);
      dpd_buf_mat_irrep_close(&G, h);
    }
  dpd_scm(&G, 0.5, 0, outfile);
  dpd_buf_close(&G);
  
  dpd_oe_file_mat_close(&g);
  dpd_oe_file_close(&g);
  dpd_oe_file_mat_close(&T1);
  dpd_oe_file_close(&T1);


  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GCiAb", 0, outfile);
  /* t(M,C) L(Mi,Ab) */
  dpd_buf_init(&L, CC_LAMPS, 0, 5, 0, 5, 0, "LIjAb", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract212(&T1, &L, &G, 0, 0, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* l(M,C) Tau(Mi,Ab) */
  dpd_buf_init(&T, CC_TAMPS, 0, 5, 0, 5, 0, "tauIjAb", 0, outfile);
  dpd_oe_file_init(&L1, CC_OEI, 0, 1, "LIA", 0, outfile);
  dpd_contract212(&L1, &T, &G, 0, 0, 0, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&L1);
  dpd_buf_close(&T);
  dpd_buf_close(&G);
  /* t(i,e) L(Mn,Ce) --> Z(Mn,Ci) */
  dpd_buf_init(&Z, CC_TMP0, 0, 11, 0, 11, 0, "Z(Mn,Ci)", 0, outfile);
  dpd_buf_init(&L, CC_LAMPS, 0, 5, 0, 5, 0, "LIjAb", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract221(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* -Z(Mn,Ci) Tau(Mn,Ab) --> G(Ci,Ab) */
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GCiAb", 0, outfile);
  dpd_buf_init(&T, CC_TAMPS, 0, 5, 0, 5, 0, "tauIjAb", 0, outfile);
  dpd_contract222(&Z, &T, &G, 1, 1, -1.0, 1.0, 0, outfile);
  dpd_buf_close(&T);
  dpd_buf_close(&Z);
  dpd_buf_close(&G);
  /* - V(iA,mC) T(m,b) --> Z(iA,bC) */
  dpd_buf_init(&Z, CC_TMP0, 10, 5, 10, 5, 0, "Z(iA,bC)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "ViAjB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, -1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP1, 10, 5, "Z(iC,bA)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(iC,bA)", 0, outfile);
  dpd_swap12(&Z, CC_TMP2, 11, 5, "Z(Ci,bA)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 11, 5, 11, 5, 0, "Z(Ci,bA)", 0, outfile);
  dpd_swap34(&Z, CC_TMP0, 11, 5, "Z(Ci,Ab)", 0, outfile);
  dpd_buf_close(&Z);
  /* V(ib,MC) T(M,A) --> Z(ib,AC) */
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(ib,AC)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "ViaJB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP2, 10, 5, "Z(iC,Ab)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 10, 5, 10, 5, 0, "Z(iC,Ab)", 0, outfile);
  dpd_swap12(&Z, CC_TMP1, 11, 5, "Z(Ci,Ab)", 0, outfile);
  dpd_buf_close(&Z);
  /* Z1(Ci,AB) + Z1(Ci,AB) --> G(Ci,AB) */
  dpd_buf_init(&Z1, CC_TMP0, 11, 5, 11, 5, 0, "Z(Ci,Ab)", 0, outfile);
  dpd_buf_init(&Z2, CC_TMP1, 11, 5, 11, 5, 0, "Z(Ci,Ab)", 0, outfile);
  dpd_axpy(&Z1, &Z2, 1.0, 0, outfile);
  dpd_buf_close(&Z1);
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GCiAb", 0, outfile);
  dpd_axpy(&Z2, &G, 1.0, 0, outfile);
  dpd_buf_close(&Z2);
  dpd_buf_close(&G);

  /* g(C,A) T(i,b) --> G(Ci,Ab) */
  dpd_oe_file_init(&g, CC_OEI, 1, 1, "GAE", 0, outfile);
  dpd_oe_file_mat_init(&g);
  dpd_oe_file_mat_rd(&g, 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_oe_file_mat_init(&T1);
  dpd_oe_file_mat_rd(&T1, 0, outfile);

  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GCiAb", 0, outfile);
  
  for(h=0; h < nirreps; h++) {
      dpd_buf_mat_irrep_init(&G, h);
      dpd_buf_mat_irrep_rd(&G, h, 0, outfile);

      for(row=0; row < G.params->rowtot[h]; row++) {
	  c = G.params->roworb[h][row][0];
	  i = G.params->roworb[h][row][1];
	  for(col=0; col < G.params->coltot[h]; col++) {
	      a = G.params->colorb[h][col][0];
	      b = G.params->colorb[h][col][1];

	      value = 0.0;

	      C = g.params->rowidx[c];  I = T1.params->rowidx[i];
	      Csym = g.params->psym[c]; Isym = T1.params->psym[i];
	      A = g.params->colidx[a];  B = T1.params->colidx[b];
	      Asym = g.params->qsym[a];  Bsym = T1.params->qsym[b];
	      
	      if((Csym==Asym) && (Isym==Bsym))
		  value += g.matrix[Csym][C][A] * T1.matrix[Isym][I][B];

	      G.matrix[h][row][col] -= value;
	    }
	}

      dpd_buf_mat_irrep_wrt(&G, h, 0, outfile);
      dpd_buf_mat_irrep_close(&G, h);
    }
  dpd_scm(&G, 0.5, 0, outfile);
  dpd_buf_close(&G);
  
  dpd_oe_file_mat_close(&g);
  dpd_oe_file_close(&g);
  dpd_oe_file_mat_close(&T1);
  dpd_oe_file_close(&T1);



  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GcIaB", 0, outfile);
  /* t(m,c) L(mI,aB) */
  dpd_buf_init(&L, CC_LAMPS, 0, 5, 0, 5, 0, "LiJaB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract212(&T1, &L, &G, 0, 0, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* l(m,c) Tau(mI,aB) */
  dpd_buf_init(&T, CC_TAMPS, 0, 5, 0, 5, 0, "tauiJaB", 0, outfile);
  dpd_oe_file_init(&L1, CC_OEI, 0, 1, "Lia", 0, outfile);
  dpd_contract212(&L1, &T, &G, 0, 0, 0, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&L1);
  dpd_buf_close(&T);
  dpd_buf_close(&G);
  /* t(I,E) L(mN,cE) --> Z(mN,cI) */
  dpd_buf_init(&Z, CC_TMP0, 0, 11, 0, 11, 0, "Z(mN,cI)", 0, outfile);
  dpd_buf_init(&L, CC_LAMPS, 0, 5, 0, 5, 0, "LiJaB", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract221(&L, &T1, &Z, 3, 1, 0, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&L);
  /* -Z(mN,cI) Tau(mN,aB) --> G(cI,aB) */
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GcIaB", 0, outfile);
  dpd_buf_init(&T, CC_TAMPS, 0, 5, 0, 5, 0, "tauiJaB", 0, outfile);
  dpd_contract222(&Z, &T, &G, 1, 1, -1.0, 1.0, 0, outfile);
  dpd_buf_close(&T);
  dpd_buf_close(&Z);
  dpd_buf_close(&G);
  /* - V(Ia,Mc) T(M,B) --> Z(Ia,Bc) */
  dpd_buf_init(&Z, CC_TMP0, 10, 5, 10, 5, 0, "Z(Ia,Bc)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "VIaJb", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, -1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP1, 10, 5, "Z(Ic,Ba)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(Ic,Ba)", 0, outfile);
  dpd_swap12(&Z, CC_TMP2, 11, 5, "Z(cI,Ba)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 11, 5, 11, 5, 0, "Z(cI,Ba)", 0, outfile);
  dpd_swap34(&Z, CC_TMP0, 11, 5, "Z(cI,aB)", 0, outfile);
  dpd_buf_close(&Z);
  /* V(IB,mc) T(m,a) --> Z(IB,ac) */
  dpd_buf_init(&Z, CC_TMP1, 10, 5, 10, 5, 0, "Z(IB,ac)", 0, outfile);
  dpd_buf_init(&V, CC_MISC, 10, 10, 10, 10, 0, "VIAjb", 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tia", 0, outfile);
  dpd_contract212(&T1, &V, &Z, 0, 2, 1, 1.0, 0.0, 0, outfile);
  dpd_oe_file_close(&T1);
  dpd_buf_close(&V);
  dpd_swap24(&Z, CC_TMP2, 10, 5, "Z(Ic,aB)", 0, outfile);
  dpd_buf_close(&Z);
  dpd_buf_init(&Z, CC_TMP2, 10, 5, 10, 5, 0, "Z(Ic,aB)", 0, outfile);
  dpd_swap12(&Z, CC_TMP1, 11, 5, "Z(cI,aB)", 0, outfile);
  dpd_buf_close(&Z);
  /* Z1(cI,aB) + Z2(cI,aB) --> G(cI,aB) */
  dpd_buf_init(&Z1, CC_TMP0, 11, 5, 11, 5, 0, "Z(cI,aB)", 0, outfile);
  dpd_buf_init(&Z2, CC_TMP1, 11, 5, 11, 5, 0, "Z(cI,aB)", 0, outfile);
  dpd_axpy(&Z1, &Z2, 1.0, 0, outfile);
  dpd_buf_close(&Z1);
  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GcIaB", 0, outfile);
  dpd_axpy(&Z2, &G, 1.0, 0, outfile);
  dpd_buf_close(&Z2);
  dpd_buf_close(&G);

  /* g(c,a) T(I,B) --> G(cI,aB) */
  dpd_oe_file_init(&g, CC_OEI, 1, 1, "Gae", 0, outfile);
  dpd_oe_file_mat_init(&g);
  dpd_oe_file_mat_rd(&g, 0, outfile);
  dpd_oe_file_init(&T1, CC_OEI, 0, 1, "tIA", 0, outfile);
  dpd_oe_file_mat_init(&T1);
  dpd_oe_file_mat_rd(&T1, 0, outfile);

  dpd_buf_init(&G, CC_GAMMA, 11, 5, 11, 5, 0, "GcIaB", 0, outfile);
  
  for(h=0; h < nirreps; h++) {
      dpd_buf_mat_irrep_init(&G, h);
      dpd_buf_mat_irrep_rd(&G, h, 0, outfile);

      for(row=0; row < G.params->rowtot[h]; row++) {
	  c = G.params->roworb[h][row][0];
	  i = G.params->roworb[h][row][1];
	  for(col=0; col < G.params->coltot[h]; col++) {
	      a = G.params->colorb[h][col][0];
	      b = G.params->colorb[h][col][1];

	      value = 0.0;

	      C = g.params->rowidx[c];  I = T1.params->rowidx[i];
	      Csym = g.params->psym[c]; Isym = T1.params->psym[i];
	      A = g.params->colidx[a];  B = T1.params->colidx[b];
	      Asym = g.params->qsym[a];  Bsym = T1.params->qsym[b];
	      
	      if((Csym==Asym) && (Isym==Bsym))
		  value += g.matrix[Csym][C][A] * T1.matrix[Isym][I][B];

	      G.matrix[h][row][col] -= value;
	    }
	}

      dpd_buf_mat_irrep_wrt(&G, h, 0, outfile);
      dpd_buf_mat_irrep_close(&G, h);
    }
  dpd_scm(&G, 0.5, 0, outfile);
  dpd_buf_close(&G);
  
  dpd_oe_file_mat_close(&g);
  dpd_oe_file_close(&g);
  dpd_oe_file_mat_close(&T1);
  dpd_oe_file_close(&T1);
}
