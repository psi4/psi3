...
void G_build(int L_irr) {
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    dpd_file2_init(&GMI, CC_GLG, L_irr, 0, 0, "GMI");
    dpd_file2_init(&Gmi, CC_GLG, L_irr, 0, 0, "Gmi");

    /* T2(MJ,AB) * L2(IJ,AB) --> G(M,I) */
    dpd_buf4_init(&tIJAB, CC_TAMPS, 0, 0, 7, 2, 7, 0, "tIJAB");
    dpd_buf4_init(&LIJAB, CC_GLG, L_irr, 0, 7, 2, 7, 0, "LIJAB");
    ...
    dpd_buf4_init(&LIjAb, CC_GLG, L_irr, 0, 5, 0, 5, 0, "LIjAb");
    ...
    dpd_file2_init(&GAE, CC_GLG, L_irr, 1, 1, "GAE");
    dpd_file2_init(&Gae, CC_GLG, L_irr, 1, 1, "Gae");

    /* T2(IJ,AB) * L2(IJ,EB) --> G(A,E) */
    dpd_buf4_init(&tIJAB, CC_TAMPS, 0, 2, 5, 2, 7, 0, "tIJAB");
    dpd_buf4_init(&LIJAB, CC_GLG, L_irr, 2, 5, 2, 7, 0, "LIJAB");
    ...
    dpd_file2_init(&GMI, CC_GLG, L_irr, 0, 0, "GMI");
    dpd_file2_init(&Gmi, CC_GLG, L_irr, 2, 2, "Gmi");
    ...
    dpd_buf4_init(&LIjAb, CC_GLG, L_irr, 22, 28, 22, 28, 0, "LIjAb");
    ...
    dpd_buf4_init(&Lijab, CC_GLG, L_irr, 10, 17, 12, 17, 0, "Lijab");
    dpd_contract442(&tijab, &Lijab, &Gmi, 0, 0, 1, 0);
    dpd_buf4_close(&tijab);
    dpd_buf4_close(&Lijab); 
    ...
    dpd_buf4_init(&LiJaB, CC_GLG, L_irr, 23, 29, 23, 29, 0, "LiJaB");
    dpd_contract442(&tiJaB, &LiJaB, &Gmi, 0, 0, 1, 1);
    dpd_buf4_close(&tiJaB);
    dpd_buf4_close(&LiJaB);
    ...
    dpd_file2_init(&GAE, CC_GLG, L_irr, 1, 1, "GAE");
    dpd_file2_init(&Gae, CC_GLG, L_irr, 3, 3, "Gae");
    ...
    dpd_buf4_init(&Lijab, CC_GLG, L_irr, 12, 15, 12, 17, 0, "Lijab");
    dpd_contract442(&Lijab, &tijab, &Gae, 3, 3, -1, 0);
    dpd_buf4_close(&tijab);
    dpd_buf4_close(&Lijab);
