#include <stdio.h>
#include <iwl.h>
#include <dpd.h>
#include <psifiles.h>
#define EXTERN
#include "globals.h"

/* DEANTI(): Convert the two-particle density from Dirac to Mulliken
** ordering.  The original, Fock-adjusted density (see the comments in
** fock.c) corresponds to a two-electron energy (or energy derivative)
** expression of the form:
**
** E(TWO) = 1/4 sum_pqrs Gpqrs <pq||rs>
**
** However, the derivative two-electron integrals are produced in
** Mulliken-ordered, symmetric form rather than Dirac-ordered
** antisymmetric form.  This code alters the two-particle density
** matrix ordering for the energy expression of the form:
**
** E(TWO) = 1/2 sum_pqrs Gpqrs <pq|rs>
**
** The final conversion to Mulliken ordering is taken care of in
** dump.c
**
** The second equation above may be derived via
**
** E(TWO) = 1/4 sum_pqrs Gpqrs (<pq|rs> - <pq|sr>)
**        = 1/4 sum_pqrs Gpqrs <pq|rs> - 1/4 sum_pqrs Gpqrs <pq|sr>
**        = 1/4 sum_pqrs Gpqrs <pq|rs> - 1/4 sum_pqrs Gpqsr <pq|rs>
**        = 1/4 sum_pqrs (Gpqrs - Gpqsr) <pq|rs>
**        = 1/2 sum_pqrs Gpqrs <pq|rs>
**
** Equations for the individual components are given explicitly in
** comments below.
** */

void deanti(void)
{
  struct dpdbuf G1, G2;
  struct oe_dpdfile D, F;
  double one_energy = 0.0, two_energy = 0.0, total_two_energy = 0.0;
  struct dpdbuf A, B, C, DInts, E, FInts;

  fprintf(outfile, "\n\tEnergies re-computed from Mulliken density:\n");
  fprintf(outfile,   "\t-------------------------------------------\n");

  dpd_oe_file_init(&D, CC_OEI, 0, 0, "DIJ", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 0, "h(i,j)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 0, 0, "Dij", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 0, "h(i,j)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 1, 1, "DAB", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 1, 1, "h(a,b)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 1, 1, "Dab", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 1, 1, "h(a,b)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 0, 1, "DIA", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 1, "h(i,a)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 0, 1, "Dia", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 1, "h(i,a)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 0, 1, "DAI", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 1, "h(i,a)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  dpd_oe_file_init(&D, CC_OEI, 0, 1, "Dai", 0, outfile);
  dpd_oe_file_init(&F, CC_OEI, 0, 1, "h(i,a)", 0, outfile);
  one_energy += dpd_oe_dot(&D, &F, 0, outfile);
  dpd_oe_file_close(&F);
  dpd_oe_file_close(&D);

  fprintf(outfile, "\tOne-electron energy        = %20.15f\n", one_energy);
  fflush(outfile);

  /* G(Ij,Kl) <-- 1/2 G(IJ,KL) + 1/2 G(ij,kl) + 1/2 G(Ij,Kl) + 1/2 G(iJ,kL) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 0, 0, 0, 0, "GIjKl", 0, outfile);
  dpd_swap12(&G1, CC_TMP0, 0, 0, "GjIKl", 0, outfile);
  dpd_buf_init(&G2, CC_TMP0, 0, 0, 0, 0, 0, "GjIKl", 0, outfile);
  dpd_swap34(&G2, CC_TMP0, 0, 0, "GiJkL", 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 0, 0, 0, 0, "GiJkL", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 0, 0, 2, 2, 0, "GIJKL", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 0, 0, 2, 2, 0, "Gijkl", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_scm(&G1, 0.5, 0, outfile);
  
  dpd_buf_init(&A, CC_AINTS, 0, 0, 0, 0, 0, "A <ij|kl>", 0, outfile);
  two_energy = dpd_dot(&A, &G1, 0, outfile);
  dpd_buf_close(&A);
  fprintf(outfile, "\tIJKL energy                = %20.15f\n", two_energy);
  total_two_energy += two_energy;
  dpd_buf_close(&G1);

  /* G(IJ,KA) <-- G(IJ,KA) + G(ij,ka) + G(Ij,Ka) + G(iJ,kA) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 10, 0, 10, 0, "GIjKa", 0, outfile);
  dpd_buf_init(&G2, CC_GAMMA, 0, 10, 0, 10, 0, "GiJkA", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 0, 10, 2, 10, 0, "GIJKA", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 0, 10, 2, 10, 0, "Gijka", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);

  dpd_buf_init(&E, CC_EINTS, 0, 10, 0, 10, 0, "E <ij|ka>", 0, outfile);
  two_energy = dpd_dot(&E, &G1, 0, outfile);
  dpd_buf_close(&E);
  fprintf(outfile, "\tIJKA energy                = %20.15f\n", 2*two_energy);
  total_two_energy += 2*two_energy;
  dpd_buf_close(&G1);

  /* G(Ij,Ab) <-- G(Ij,Ab) - G(Ib,jA) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 5, 0, 5, 0, "GIjAb", 0, outfile);
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "GIbjA", 0, outfile);
  dpd_buf_sort(&G2, CC_TMP0, prsq, 0, 5, "GIbjA (Ij,Ab)", 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "GIbjA (Ij,Ab)", 0, outfile);
  dpd_axpy(&G2, &G1, -1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_close(&G1);
  
  /* G(IJ,AB) <-- G(IJ,AB) - G(IB,JA) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 5, 2, 7, 0, "GIJAB", 0, outfile);
  dpd_copy(&G1, CC_TMP0, "G(IJ,AB)", 0, outfile);
  dpd_buf_close(&G1);
  dpd_buf_init(&G1, CC_TMP0, 0, 5, 0, 5, 0, "G(IJ,AB)", 0, outfile);
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "GIBJA", 0, outfile);
  dpd_buf_sort(&G2, CC_TMP0, prsq, 0, 5, "GIBJA (IJ,AB)", 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "GIBJA (IJ,AB)", 0, outfile);
  dpd_axpy(&G2, &G1, -1.0, 0, outfile); 
  dpd_buf_close(&G2);
  dpd_buf_close(&G1);

  /* G(ij,ab) <-- G(ij,ab) - G(ib,ja) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 5, 2, 7, 0, "Gijab", 0, outfile);
  dpd_copy(&G1, CC_TMP0, "G(ij,ab)", 0, outfile);
  dpd_buf_close(&G1);
  dpd_buf_init(&G1, CC_TMP0, 0, 5, 0, 5, 0, "G(ij,ab)", 0, outfile);
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "Gibja", 0, outfile);
  dpd_buf_sort(&G2, CC_TMP0, prsq, 0, 5, "Gibja (ij,ab)", 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "Gibja (ij,ab)", 0, outfile);
  dpd_axpy(&G2, &G1, -1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_close(&G1);

  /* G(IJ,AB) <-- G(IJ,AB) + G(ij,ab) + G(Ij,Ab) + G(iJ,aB) */
  dpd_buf_init(&G1, CC_GAMMA, 0, 5, 0, 5, 0, "GIjAb", 0, outfile);
  dpd_buf_sort(&G1, CC_TMP0, qpsr, 0, 5, "G(jI,bA)", 0, outfile);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "G(jI,bA)", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "G(IJ,AB)", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP0, 0, 5, 0, 5, 0, "G(ij,ab)", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);

  dpd_buf_init(&DInts, CC_DINTS, 0, 5, 0, 5, 0, "D <ij|ab>", 0, outfile);
  two_energy = dpd_dot(&DInts, &G1, 0, outfile);
  dpd_buf_close(&DInts);
  fprintf(outfile, "\tIJAB energy                = %20.15f\n", two_energy);
  total_two_energy += two_energy;
  dpd_buf_close(&G1);
  
  /* G(IB,JA) <-- G(IB,JA) + G(ib,ja) + G(Ib,Ja) + G(iB,jA) */
  dpd_buf_init(&G1, CC_GAMMA, 10, 10, 10, 10, 0, "GIBJA", 0, outfile);
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "Gibja", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2); 
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "GIbJa", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile); 
  dpd_buf_close(&G2); 
  dpd_buf_init(&G2, CC_GAMMA, 10, 10, 10, 10, 0, "GiBjA", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile); 
  dpd_buf_close(&G2);
  dpd_buf_init(&C, CC_CINTS, 10, 10, 10, 10, 0, "C <ia|jb>", 0, outfile);
  two_energy = dpd_dot(&C, &G1, 0, outfile);
  dpd_buf_close(&C);
  fprintf(outfile, "\tIBJA energy                = %20.15f\n", two_energy);
  total_two_energy += two_energy;
  dpd_buf_close(&G1);

  /* G(CI,AB) <-- G(CI,AB) + G(ci,ab) + G(Ci,Ab) + G(cI,aB) */
  dpd_buf_init(&G1, CC_GAMMA, 11, 5, 11, 5, 0, "GCiAb", 0, outfile); 
  dpd_buf_init(&G2, CC_GAMMA, 11, 5, 11, 5, 0, "GcIaB", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2); 
  dpd_buf_init(&G2, CC_GAMMA, 11, 5, 11, 7, 0, "GCIAB", 0, outfile); 
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);  
  dpd_buf_close(&G2); 
  dpd_buf_init(&G2, CC_GAMMA, 11, 5, 11, 7, 0, "Gciab", 0, outfile); 
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);

  dpd_buf_init(&FInts, CC_FINTS, 10, 5, 10, 5, 0, "F <ia|bc>", 0, outfile);
  dpd_swap12(&FInts, CC_TMP0, 11, 5, "F(CI,BA)", 0, outfile);
  dpd_buf_close(&FInts);
  dpd_buf_init(&FInts, CC_TMP0, 11, 5, 11, 5, 0, "F(CI,BA)", 0, outfile);
  dpd_swap34(&FInts, CC_TMP1, 11, 5, "F(CI,AB)", 0, outfile);
  dpd_buf_close(&FInts);
  dpd_buf_init(&FInts, CC_TMP1, 11, 5, 11, 5, 0, "F(CI,AB)", 0, outfile);
  two_energy = dpd_dot(&FInts, &G1, 0, outfile);
  dpd_buf_close(&FInts);
  fprintf(outfile, "\tCIAB energy                = %20.15f\n", 2*two_energy);
  total_two_energy += 2*two_energy;
  dpd_buf_close(&G1);

  /* G(Ab,Cd) <-- 1/2 G(AB,CD) + 1/2 G(ab,cd) + G(Ab,Cd) */
  dpd_buf_init(&G1, CC_GAMMA, 5, 5, 5, 5, 0, "GAbCd", 0, outfile);
  dpd_swap12(&G1, CC_TMP0, 5, 5, "G(bA,Cd)", 0, outfile);
  dpd_buf_init(&G2, CC_TMP0, 5, 5, 5, 5, 0, "G(bA,Cd)", 0, outfile);
  dpd_swap34(&G2, CC_TMP1, 5, 5, "G(aB,cD)", 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_TMP1, 5, 5, 5, 5, 0, "G(aB,cD)", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 5, 5, 7, 7, 0, "GABCD", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_buf_init(&G2, CC_GAMMA, 5, 5, 7, 7, 0, "Gabcd", 0, outfile);
  dpd_axpy(&G2, &G1, 1.0, 0, outfile);
  dpd_buf_close(&G2);
  dpd_scm(&G1, 0.5, 0, outfile);
  
  dpd_buf_init(&B, CC_BINTS, 5, 5, 5, 5, 0, "B <ab|cd>", 0, outfile);
  two_energy = dpd_dot(&B, &G1, 0, outfile);
  dpd_buf_close(&B);
  fprintf(outfile, "\tABCD energy                = %20.15f\n", two_energy);
  total_two_energy += two_energy;
  dpd_buf_close(&G1);

  fprintf(outfile, "\tTotal two-electron energy  = %20.15f\n", total_two_energy);
  fprintf(outfile, "\tCCSD correlation energy    = %20.15f\n",
	  one_energy + total_two_energy);
  fprintf(outfile, "\tTotal CCSD energy          = %20.15f\n",
	  one_energy + total_two_energy + moinfo.eref);
}
