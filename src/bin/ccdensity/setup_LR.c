...
void setup_LR(void)
{
  dpdfile2 L1,R1;
  dpdbuf4 L2,R2;
  int i,j,L_irr, R_irr, L_root, R_root;
  ...
  params.G_irr = params.L_irr ^ params.R_irr;
  ...
  /* copy L from CC_LAMPS into CC_GLG for ground state density code */
  if (params.ref == 0) {
  ...
  /* for excited states, make additional copy of L in CC_GL, since
     CC_GLG will change for use with ground-state code (see below) */
  if (!params.ground) {
    if (params.ref == 0) {
    ...
    if ( (params.ref==0) || (params.ref==1) ) {
      ...
      dpd_file2_copy(&L1, CC_GL, "LIA");
      dpd_file2_close(&L1);
      dpd_file2_init(&L1, CC_GLG, L_irr, 2, 3, "Lia");
      dpd_file2_copy(&L1, CC_GL, "Lia");
      dpd_file2_close(&L1);
      dpd_buf4_init(&L2, CC_GLG, L_irr, 2, 7, 2, 7, 0, "LIJAB");
      dpd_buf4_copy(&L2, CC_GL, "LIJAB");
      dpd_buf4_close(&L2);
      dpd_buf4_init(&L2, CC_GLG, L_irr, 12, 17, 12, 17, 0, "Lijab");
      dpd_buf4_copy(&L2, CC_GL, "Lijab");
      dpd_buf4_close(&L2);
      dpd_buf4_init(&L2, CC_GLG, L_irr, 22, 28, 22, 28, 0, "LIjAb");
      dpd_buf4_copy(&L2, CC_GL, "LIjAb");
      dpd_buf4_close(&L2);
    }
  }

  /* for excited states, scale L for ground-state density contributions
   to R0 * L for now and ultimately R0*L + Zeta for relaxed density */
  if (!params.ground) {
    if ( (params.ref==0) || (params.ref==1) ) {
      dpd_file2_init(&L1, CC_GLG, L_irr, 0, 1, "LIA");
  ...
  if( (params.ref==0) || (params.ref==1) ) { /** RHF/ROHF **/
    ...
    dpd_buf4_close(&L2);
    /* Build L2IAJB List */
    dpd_buf4_init(&L2, CC_GLG, L_irr, 0, 5, 2, 7, 0, "LIJAB");
    ...
    dpd_buf4_init(&L2, CC_GLG, L_irr, 10, 10, 10, 10, 0, "LIAjb");
    ...
    dpd_buf4_init(&L2, CC_GLG, L_irr, 10, 15, 12, 17, 0, "Lijab");
    ...
    dpd_buf4_init(&L2, CC_GLG, L_irr, 23, 29, 23, 29, 0, "LiJaB");
    dpd_buf4_sort(&L2, CC_GLG, prqs, 30, 20, "LiaJB");
    dpd_buf4_close(&L2);

    dpd_buf4_init(&L2, CC_GLG, L_irr, 20, 30, 20, 30, 0, "LIAjb");
