...
/*  term 16 XIA += 0.5 Lioaf (Rmnef Wmnoe) */
    ...
    dpd_buf4_init(&H2, CC_HBAR, 0, 2, 11, 2, 11, 0, "WMNIE");
...
/*  term 10 XIA += 0.5 (Lmnef Rmneg) Wfiga */
    ...
    dpd_file2_init(&I1, EOM_TMP, G_irr, 1, 1, "LR2_VV");
    dpd_buf4_init(&H2, CC_HBAR, 0, 10, 5, 10, 7, 0, "WAMEF");
    dpd_dot23(&I1, &H2, &XIA, 0, 0, 1.0, 1.0);
    dpd_buf4_close(&H2);
    dpd_file2_close(&I1);
    dpd_file2_init(&I1, EOM_TMP, G_irr, 1, 1, "LR2_vv");
...
/* term 12, + (Rmnef Lmieg) Wgnaf */
    ...
    dpd_buf4_init(&I2, EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OVOV");
    dpd_buf4_sort(&I2, EOM_TMP1, psrq, 10, 10, "Z (NG,IF)"); 
    dpd_buf4_close(&I2);
    dpd_buf4_init(&H2, CC_HBAR, 0, 10, 5, 10, 7, 0, "WAMEF");
    dpd_buf4_init(&I2, EOM_TMP1, G_irr, 10, 10, 10, 10, 0, "Z (NG,IF)");
    ...
    dpd_buf4_init(&I2, EOM_TMP, G_irr, 10, 10, 10, 10, 0, "R2L2_OvOv");
    dpd_buf4_sort(&I2, EOM_TMP1, psqr, 10, 11, "Z (Ng,fI)"); 
    dpd_buf4_close(&I2);
    dpd_buf4_init(&H2, CC_HBAR, 0, 10, 5, 10, 5, 0, "WaMeF");
    dpd_buf4_init(&I2, EOM_TMP1, G_irr, 10, 11, 10, 11, 0, "Z (Ng,fI)");
    dpd_contract442(&I2, &H2, &XIA, 3, 3, -1.0, 1.0);
    dpd_buf4_close(&I2);
    dpd_buf4_close(&H2);
