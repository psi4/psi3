...
void sort_rtd_uhf(struct RHO_Params S)
{
  ...
  for(h=0; h < nirreps; h++) {
    for(i=0; i < aoccpi[h]; i++) {
      ...
      for(j=0; j < aoccpi[h^S.R_irr]; j++) {
        J = qt_aocc[aocc_off[h^S.R_irr] + j];
  ...
  for(h=0; h < nirreps; h++) {
    for(a=0; a < avirtpi[h]; a++) {
      ...
      for(b=0; b < avirtpi[h^S.R_irr]; b++) {
        B = qt_avir[avir_off[h^S.R_irr] + b];
  ...
  for(h=0; h < nirreps; h++) {
    for(i=0; i < aoccpi[h]; i++) {
      I = qt_aocc[aocc_off[h] + i];
      for(a=0; a < avirtpi[h^S.R_irr]; a++) {
        A = qt_avir[avir_off[h^S.R_irr] + a];
  ...
  dpd_file2_init(&D, CC_TMP, S.R_irr, 2, 2, "RTDij");
  dpd_file2_mat_init(&D); 
  dpd_file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(i=0; i < boccpi[h]; i++) { 
      I = qt_bocc[bocc_off[h] + i];
      for(j=0; j < boccpi[h^S.R_irr]; j++) {
        J = qt_bocc[bocc_off[h^S.R_irr] + j];
  ...
  dpd_file2_init(&D, CC_TMP, S.R_irr, 3, 3, "RTDab");
  dpd_file2_mat_init(&D);
  dpd_file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(a=0; a < bvirtpi[h]; a++) {
      A = qt_bvir[bvir_off[h] + a];
      for(b=0; b < bvirtpi[h^S.R_irr]; b++) {
        B = qt_bvir[bvir_off[h^S.R_irr] + b];
  ...
  /* Note that this component of the density is stored occ-vir */
  dpd_file2_init(&D, CC_TMP, S.R_irr, 2, 3, "RTDai");
  dpd_file2_mat_init(&D);
  dpd_file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(i=0; i < boccpi[h]; i++) {
      I = qt_bocc[bocc_off[h] + i];
      for(a=0; a < bvirtpi[h^S.R_irr]; a++) {
        A = qt_bvir[bvir_off[h^S.R_irr] + a];
  ...
  dpd_file2_init(&D, CC_TMP, S.R_irr, 2, 3, "RTDia");
  dpd_file2_mat_init(&D);
  dpd_file2_mat_rd(&D);
  for(h=0; h < nirreps; h++) {
    for(i=0; i < boccpi[h]; i++) {
