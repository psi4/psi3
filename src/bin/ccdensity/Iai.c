...
void Iai(void)
{
  ...
  struct dpdbuf G, Eints, Dints, Cints, Fints, Bints;
  ...
  dpd_oe_file_init(&D, CC_OEI, 0, 0, "DIJ", 0, outfile);
  dpd_buf_init(&Eints, CC_EINTS, 11, 0, 11, 0, 1, "E <ai|jk>", 0, outfile);
  dpd_dot24(&D, &Eints, &I, 0, 0, 1.0, 1.0, 0, outfile);
  dpd_dot24(&D, &Eints, &I, 1, 0, 1.0, 1.0, 0, outfile);
  dpd_buf_close(&Eints);
  dpd_oe_file_close(&D);
  ...
  dpd_oe_file_init(&D, CC_OEI, 0, 1, "DIA", 0, outfile);
  dpd_dot14(&D, &Cints, &I, 0, 0, -1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&D);
  dpd_oe_file_init(&D, CC_OEI, 0, 1, "DAI", 0, outfile);
  dpd_dot14(&D, &Cints, &I, 0, 0, -1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&D);
  ...
  dpd_oe_file_init(&D, CC_OEI, 0, 1, "Dia", 0, outfile);
  dpd_dot24(&D, &Dints, &I, 0, 1, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&D);
  dpd_oe_file_init(&D, CC_OEI, 0, 1, "Dai", 0, outfile);
  dpd_dot24(&D, &Dints, &I, 0, 1, 1.0, 1.0, 0, outfile);
  dpd_oe_file_close(&D);
  ...
  dpd_dot24(&D, &Fints, &I, 1, 1, 1.0, 1.0, 0, outfile);
  dpd_dot24(&D, &Fints, &I, 0, 1, 1.0, 1.0, 0, outfile);
  dpd_buf_close(&Fints);
  dpd_oe_file_close(&D);
  ...
  dpd_buf_init(&Eints, CC_EINTS, 11, 2, 11, 0, 1, "E <ai|jk>", 0, outfile);
  ...
  dpd_contract122(&Cints, &G, &I, 1, 1, 2.0, 1.0, 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_close(&Cints);
  ...
  dpd_buf_init(&Dints, CC_DINTS, 0, 5, 0, 5, 0, "D <ij|ab>", 0, outfile);
  dpd_buf_sort(&Dints, CC_TMP0, rqps, 11, 10, "D <ij|ab> (aj,ib)", 0, outfile);
  dpd_buf_close(&Dints);
  dpd_buf_init(&Dints, CC_TMP0, 11, 10, 11, 10, 0, "D <ij|ab> (aj,ib)",
	       0, outfile);
  dpd_buf_init(&G, CC_GAMMA, 0, 10, 0, 10, 0, "GIjKa", 0, outfile);
  ...
  dpd_contract122(&Fints, &G, &I, 1, 1, 2.0, 1.0, 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_close(&Fints);

  dpd_buf_init(&Fints, CC_FINTS, 10, 5, 10, 5, 0, "F <ia|bc>", 0, outfile);
  dpd_buf_init(&G, CC_GAMMA, 0, 5, 0, 5, 0, "GIjAb", 0, outfile);
  dpd_buf_sort(&G, CC_TMP0, qpsr, 0, 5, "GiJaB", 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_init(&G, CC_TMP0, 0, 5, 0, 5, 0, "GiJaB", 0, outfile);
  dpd_contract122(&Fints, &G, &I, 1, 1, 2.0, 1.0, 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_close(&Fints);
  ...
  dpd_buf_init(&G, CC_GAMMA, 10, 10, 10, 10, 0, "GIbjA", 0, outfile);
  dpd_buf_sort(&G, CC_TMP0, rsqp, 10, 11, "GIbjA (jA,bI)", 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_init(&G, CC_TMP0, 10, 11, 10, 11, 0, "GIbjA (jA,bI)", 0, outfile);
  dpd_buf_init(&Fints, CC_FINTS, 10, 5, 10, 5, 0, "F <ia|bc>", 0, outfile);
  ...
  dpd_buf_sort(&G, CC_TMP0, qpsr, 10, 5, "GIcAb", 0, outfile);
  dpd_buf_close(&G);
  dpd_buf_init(&G, CC_TMP0, 10, 5, 10, 5, 0, "GIcAb", 0, outfile);
