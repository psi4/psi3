/******************************************************************************
	...
	IRREP.CC
        ...
        symmetrizes set of delocalized internal coordinates
        ...
        returns:
          -- pointer to 'symm_coordinate', a vector containing pointers to
	     properly symmetrized delocalized internal coordinate vectors

  note: if large molecule optimizations are desired keeping both 'coord_tmp'
    and 'coord_tmp2' until end may be undesirable, this is only necessary
    to avoid directly copying 'coord_tmp2' into 'symm_coord', which isn't a big
    deal
...
double **irrep(internals &simples, double **di_coord) {
  ...
  int i, j, k, a, ops, coord_num, irrep, offset,index, /* counter variables */
      order,                                           /* order of molecular point group */
...
double norm, dot,                                      /* norm and dot product of coordinate */
       ...
       **coord_tmp, **coord_tmp2,                      /* temporary storage of coordinates */
       **irreps_spanned;                               /* irrep reduction coefficients, coordinates in rows,
							  irreps in columns */
  ...
  irreps_spanned = init_matrix(num_nonzero,syminfo.num_irreps);
  ...
  if (optinfo.print_delocalize == 1) {
     fprintf(outfile,"\n\nIrrep info for unsymmetrized coordinates (each row a coordinate, each column an irrep):\n");
     print_mat(irreps_spanned,num_nonzero,syminfo.num_irreps,outfile);
  ...
  /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    need to use projection operator for coordinates that span more than one irrep 
  ...
  if(num_spanned == num_nonzero) {
    if(optinfo.print_delocalize == 1) {
	fprintf(outfile,"\nCoordinates already symmetry adapted.  No need for projection or orthogonalization.\n");
      }
    for(coord_num=0;coord_num<num_nonzero;coord_num++) { 
	for(irrep=0;irrep<syminfo.num_irreps;++irrep) 
          if(irreps_spanned[coord_num][irrep] > SPANNED_IRREP_TOL)
	...
	irreps_spanned = irrep_reduce(coord_tmp, simples, num_spanned_big, order);
       ...
       if (optinfo.print_delocalize == 1) {
          ...
          print_mat(irreps_spanned,num_spanned_big,syminfo.num_irreps,outfile);
	  ...
	  punt("projected coordinate(s) span incorrect number of irreps");
       ...
       /*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 now we need to orthogonalize the vectors
	       ...
	       if (irr_tmp[coord_num] == irrep ) {
		       ...
		       for (i=1;i<(index+1);++i) {
			       ...
			       tmp_evect[j] -= dot * coord_tmp2[offset+index-i][j];
                       ...
                       /* find norm of evect_tmp, keep if big enough (since some vectors may be linearly dependent
		       so there could be small vectors */
		       norm=0.0;
		       for (i=0;i<simples.get_num();++i) {
...
/*---------------------------------------------------------------------
  ...
  finds reduction coefficients of delocalized coordinates
  ...
  returns:    characters -- matrix of characters, one row per coordinate 
                            one irrep per column			    
----------------------------------------------------------------------*/			    

double **irrep_reduce(double **coord_mat, internals &simples, int num_coords, int order) {

  int coord, operations, irreducible, i, j, a; /* counter variables */
  ...
  chars = init_array(syminfo.num_irreps);
  tmp_vec = init_array(simples.get_num());
  coef_mat = init_matrix(num_coords,syminfo.num_irreps);
  
  /* loop over number of coordinate vectors */
  for(coord=0;coord<num_coords;++coord) { 
      
    zero_arr(chars,syminfo.num_irreps);

    /* determine the character of the vector under different operations */
    for (operations = 0;operations<syminfo.num_irreps;++operations) {
    ...
    for (irreducible=0;irreducible<syminfo.num_irreps;++irreducible) {
      red_coef=0.0;
      for (j=0;j<syminfo.num_irreps;++j) {
