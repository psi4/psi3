/*! \file
    \ingroup OPTKING
    \brief disp_grad_energy_cart(): makes symmetric cartesian displacements for
     in order to determine gradients from energy computations
*/
...
{
  int i,j,a,b,I,k,dim, nsalc_all, natom, atom, xyz, cnt, loner, *ndisp;
  ...
  // we are only interested in irrep=0 symmetric modes
  irrep = 0;
  ...
  geoms = init_matrix(ndisp[0],3*natom);
  ...
  if (optinfo.points == 3) {
    ...
    for (i=0; i<nsalc[irrep]; ++i) {
      cnt += 2;
      for (j=0; j < 3*natom; ++j) {
        geoms[cnt][j]   -= optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
        geoms[cnt+1][j] += optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
      }
    }
  }
  else if (optinfo.points == 5) {
    ...
    for (i=0; i<nsalc[irrep]; ++i) {
      cnt += 4;
      for (j=0; j < 3*natom; ++j) {
        geoms[cnt][j]   -= 2.0 * optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
        geoms[cnt+1][j] -= 1.0 * optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
        geoms[cnt+2][j] += 1.0 * optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
        geoms[cnt+3][j] += 2.0 * optinfo.disp_size * salc[irrep][i][j] / sqrt(masses[j]);
      }
    }
  }
  free_array(masses);
  ...
  if (ndisp[0]) {
    psio_write(PSIF_OPTKING, "OPT: Displaced geometries",
      (char *) &(geoms[0][0]), ndisp[0]*3*natom*sizeof(double), next, &next);
    if (print) {
      fprintf(outfile,"Displaced Geometries of irrep %d.\n", irrep);
      print_mat(geoms, ndisp[0], 3*natom, outfile);
    }
  }
  ...
  psio_write_entry(PSIF_OPTKING, "OPT: Irrep per disp",
      (char *) &(irrep_per_disp[0]), ndisp[0]*sizeof(int));
  ...
  fprintf(outfile,"Number of %s displaced geometries is %d.\n",syminfo.irrep_lbls[0],ndisp[0]);


  if(optinfo.external_energies){
    const char *sym;
    fprintf(outfile,"Writing reference and displaced geometry to text file.\n");
    ...
    for(i=0; i<ndisp[0]; i++) {
      ...
      for(int n = 0; n<natom; n++){
        ...
        fprintf(fp_dispcart,"%s %16.10lf %16.10lf %16.10lf\n", sym, 
          geoms[i][3*n],geoms[i][3*n+1],geoms[i][3*n+2]);
      }
    }
    fclose(fp_dispcart);
  }
