...
int main(int argc, char * argv[]) {
...
#ifdef TIME_DF_MP2
  if (myproc == 0) timer_on("Form mo_p_ia");
#endif
...
#ifdef TIME_DF_MP2
  if (myproc == 0) timer_off("Form mo_p_ia");
#endif
...
#ifdef TIME_DF_MP2
  if (myproc == 0) timer_off("Form B_ia^P");
#endif
  ...
  for (int i=0; i < nact_docc; ++i) {
    for (int j=0; j <= i; ++j) {
      ...
      #ifdef TIME_DF_MP2
        if (myproc == 0) timer_on("Construct I");
      #endif
      int ia, jb;
