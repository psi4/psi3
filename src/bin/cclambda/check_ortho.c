      ...
      for (root_L_irr=0; root_L_irr< params.Ls_per_irrep[L_irr]; ++root_L_irr) {
        ...
        for (R_irr=0; R_irr<moinfo.nirreps; ++R_irr)
          for (root_R_irr=0; root_R_irr< params.Ls_per_irrep[R_irr]; ++root_R_irr) {
            ...
            if (L_irr == R_irr)
              tval = LR_overlap_ROHF(L_irr, L_state_index, R_state_index);
            else
              tval = -99.0;
      ...
      for (root_L_irr=0; root_L_irr< params.Ls_per_irrep[L_irr]; ++root_L_irr) {
        ...
        for (R_irr=0; R_irr<moinfo.nirreps; ++R_irr)
          for (root_R_irr=0; root_R_irr< params.Ls_per_irrep[R_irr]; ++root_R_irr) {
            ...
            if (L_irr == R_irr)
              tval = LR_overlap_RHF(L_irr, L_state_index, R_state_index);
            else
              tval = -99.0;
...
double LR_overlap_ROHF(int IRR, int L_index, int R_index) {
  ...
  sprintf(L1A_lbl, "LIA %d", L_index);
  sprintf(L1B_lbl, "Lia %d", L_index);
  sprintf(L2AA_lbl, "LIJAB %d", L_index);
  sprintf(L2BB_lbl, "Lijab %d", L_index);
  sprintf(L2AB_lbl, "LIjAb %d", L_index);
...
double LR_overlap_RHF(int IRR, int L_index, int R_index) {
  ...
  sprintf(lbl, "2RIjAb - RIjbA %d", R_index);
  dpd_buf4_init(&R2, CC_RAMPS, IRR, 0, 5, 0, 5, 0, lbl);

  sprintf(lbl, "LIjAb %d", L_index);
  dpd_buf4_init(&L2, CC_LAMPS, IRR, 0, 5, 0, 5, 0, lbl);
  overlap2 = dpd_buf4_dot(&L2, &R2);
  dpd_buf4_close(&L2);
  dpd_buf4_close(&R2);

  sprintf(lbl, "2LIjAb - LIjbA %d", L_index);
  dpd_buf4_init(&L2, CC_LAMPS, IRR, 0, 5, 0, 5, 0, lbl);

  sprintf(lbl, "RIjAb %d", R_index);
  dpd_buf4_init(&R2, CC_RAMPS, IRR, 0, 5, 0, 5, 0, lbl);
