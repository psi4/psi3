...
void GaeL2(void)
{
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    /** X(IJ,AB) = <IJ||AE> G(B,E) **/
    dpd_buf4_init(&X1, CC_TMP2, 0, 2, 5, 2, 5, 0, "X(IJ,AB)");
    dpd_buf4_init(&D, CC_DINTS, 0, 2, 5, 2, 5, 0, "D <IJ||AB> (I>J,AB)");
    ...
    dpd_buf4_init(&X2, CC_TMP2, 0, 2, 5, 2, 5, 0, "X(IJ,BA)");
    dpd_buf4_axpy(&X2, &X1, -1);
    dpd_buf4_close(&X2);
    /** X(IJ,AB) --> New L(IJ,AB) **/
    dpd_buf4_init(&L2, CC_LAMPS, 0, 2, 5, 2, 7, 0, "New LIJAB");
    dpd_buf4_axpy(&X1, &L2, 1);
    dpd_buf4_close(&X1);
    dpd_buf4_close(&L2);

    /** X(ij,ab) = <ij||ae> G(b,e) **/
    dpd_buf4_init(&X1, CC_TMP2, 0, 12, 15, 12, 15, 0, "X(ij,ab)");
    ...
    /** X(ij,ab) = X(ij,ab) - X(ij,ba) **/
    dpd_buf4_init(&X2, CC_TMP2, 0, 12, 15, 12, 15, 0, "X(ij,ba)");
    dpd_buf4_axpy(&X2, &X1, -1);
    dpd_buf4_close(&X2);
    /** X(ij,ab) --> New L(ij,ab) **/
    dpd_buf4_init(&L2, CC_LAMPS, 0, 12, 15, 12, 17, 0, "New Lijab");
    dpd_buf4_axpy(&X1, &L2, 1);
    dpd_buf4_close(&X1);
    dpd_buf4_close(&L2);
...
void GmiL2(void)
{
  ...
  if(params.ref == 0 || params.ref == 1) { /** RHF/ROHF **/
    ...
    dpd_buf4_init(&X1, CC_TMP2, 0, 0, 7, 0, 7, 0, "X(0,7) 1");
    dpd_contract424(&D, &GMI, &X1, 1, 0, 1, -1.0, 0.0);
    dpd_buf4_init(&X2, CC_TMP2, 0, 0, 7, 0, 7, 0, "X(0,7) 2");
    ...
    dpd_buf4_init(&X2, CC_TMP2, 0, 0, 7, 0, 7, 0, "X(JI,AB)");
    dpd_buf4_axpy(&X2, &X1, -1);
    dpd_buf4_close(&X2);
    /** X(IJ,AB) --> New L(IJ,AB) **/
    dpd_buf4_init(&L2, CC_LAMPS, 0, 0, 7, 2, 7, 0, "New LIJAB");
    dpd_buf4_axpy(&X1, &L2, 1);
    dpd_buf4_close(&L2);
    dpd_buf4_close(&X1);
    ...
    /** X(ij,ab) = X(ij,ab) - X(ji,ab) **/
    dpd_buf4_init(&X2, CC_TMP2, 0, 10, 17, 10, 17, 0, "X(ji,ab)");
    dpd_buf4_axpy(&X2, &X1, -1);
    dpd_buf4_close(&X2);
    /** X(ij,ab) --> New L(ij,ab) **/
    dpd_buf4_init(&L2, CC_LAMPS, 0, 10, 17, 12, 17, 0, "New Lijab");
    dpd_buf4_axpy(&X1, &L2, 1);
    dpd_buf4_close(&L2);
    dpd_buf4_close(&X1);
