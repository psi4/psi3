...
void diis(int iter)
{
  ...
  dpd_file2_init(&T1, CC_TMP0, 0, 0, 1, "T1(I,A)");
  dpd_buf4_init(&T2a, CC_TMP0, 0, 2, 7, 2, 7, 0, "T2(IJ,AB)");
  dpd_buf4_init(&T2b, CC_TMP0, 0, 0, 5, 0, 5, 0, "T2(Ij,Ab)");
  for(h=0; h < nirreps; h++) {
  ...
  word=0;
  dpd_file2_init(&T1a, CC_OEI, 0, 0, 1, "New LIA");
  dpd_file2_mat_init(&T1a);
  dpd_file2_mat_rd(&T1a);
  dpd_file2_init(&T1b, CC_OEI, 0, 0, 1, "LIA");
  ...
  dpd_buf4_init(&T2a, CC_LAMPS, 0, 2, 7, 2, 7, 0, "New LIJAB");
  dpd_buf4_init(&T2b, CC_LAMPS, 0, 2, 7, 2, 7, 0, "LIJAB");
  for(h=0; h < nirreps; h++) {
  dpd_buf4_mat_irrep_init(&T2a, h); 0,
      dpd_buf4_mat_irrep_rd(&T2a, h);
  dpd_buf4_mat_irrep_init(&T2b, h); 0,
  ...
  dpd_buf4_init(&T2a, CC_LAMPS, 0, 0, 5, 0, 5, 0, "New LIjAb");
  dpd_buf4_init(&T2b, CC_LAMPS, 0, 0, 5, 0, 5, 0, "LIjAb");
  for(h=0; h < nirreps; h++) {
  dpd_buf4_mat_irrep_init(&T2a, h); 0,
      dpd_buf4_mat_irrep_rd(&T2a, h);
  dpd_buf4_mat_irrep_init(&T2b, h); 0,
