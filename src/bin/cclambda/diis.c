...
void diis(int iter)
{
  ...
  word=0;
  dpd_oe_file_init(&T1a, CC_OEI, 0, 1, "New LIA", 0, outfile);
  dpd_oe_file_mat_init(&T1a);
  dpd_oe_file_mat_rd(&T1a, 0, outfile);
  dpd_oe_file_init(&T1b, CC_OEI, 0, 1, "LIA", 0, outfile);
  ...
  dpd_buf_init(&T2a, CC_LAMPS, 2, 7, 2, 7, 0, "New LIJAB", 0, outfile);
  dpd_buf_init(&T2b, CC_LAMPS, 2, 7, 2, 7, 0, "LIJAB", 0, outfile);
  for(h=0; h < nirreps; h++) {
  ...
  dpd_buf_init(&T2a, CC_LAMPS, 0, 5, 0, 5, 0, "New LIjAb", 0, outfile);
  dpd_buf_init(&T2b, CC_LAMPS, 0, 5, 0, 5, 0, "LIjAb", 0, outfile);
  for(h=0; h < nirreps; h++) {
      dpd_buf_mat_irrep_init(&T2a, h);
      dpd_buf_mat_irrep_rd(&T2a, h, 0, outfile);
      dpd_buf_mat_irrep_init(&T2b, h);
