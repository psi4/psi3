...
\funcdesc{char *chkpt\_rd\_label()}
{Reads the main the checkpoint file label.}
{takes no arguments.}
{calculation label.} \\
\funcdesc{char *chkpt\_rd\_sym\_label()}
...
Functions that return \celem{char**}
\end{center}
\funcdesc{char **chkpt\_rd\_irr\_labs()} 
...
\funcdesc{char **chkpt\_rd\_hfsym\_labs()}
...
\funcdesc{int chkpt\_rd\_nirreps()}
...
\funcdesc{int chkpt\_rd\_nshell()}
...
\funcdesc{chkpt\_rd\_atom\_position()}
...
\funcdesc{int *chkpt\_rd\_clsdpi()}
...
\funcdesc{int *chkpt\_rd\_openpi()}
{Reads in an array which has an element for each irrep of the
point group of the molecule (n.b. not just the ones
with a non-zero number of basis functions).  Each element
...
\funcdesc{int **chkpt\_rd\_shell\_transm()}
...
\funcdesc{double *chkpt\_rd\_exps()}
{Reads in the exponents of the primitive Gaussian functions.}
{takes no arguments.}
{an array of doubles.} \\
\funcdesc{double *chkpt\_rd\_zvals()}
...
\funcdesc{double **chkpt\_rd\_ccvecs()}
...
\funcdesc{chkpt\_rd\_contr\_full()}
...
\funcdesc{double **chkpt\_rd\_geom()}
...
\funcdesc{double **chkpt\_rd\_scf()\\
...
\end{center}
{The z-matrix is read from the checkpoint file as an array of \celem{z\_entry} structs
which are declared in \file{chkpt.h}.  This structure contains the reference atom,
